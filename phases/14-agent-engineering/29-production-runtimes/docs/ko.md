...
프로덕션 에이전트는 Jupyter 노트북에서는 드러나지 않는 방식으로 실패한다: 37번째 스텝에서의 네트워크 타임아웃, 음성 통화 중간에 사용자가 끊어버림, 머신 재부팅으로 죽는 크론 작업, 메모리가 부족해지는 백그라운드 워커. 어떤 실패를 견뎌낼 수 있는지는 런타임 형태가 결정한다.
