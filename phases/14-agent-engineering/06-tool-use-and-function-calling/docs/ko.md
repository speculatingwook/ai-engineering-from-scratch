...
> Toolformer(Schick et al., 2023)는 자기지도(self-supervised) 도구 주석을 시작했다. Berkeley Function Calling Leaderboard V4(Patil et al., 2025)는 2026년 기준선을 세운다: 40% 에이전트형(agentic), 30% 멀티턴(multi-turn), 10% 라이브(live), 10% 논라이브(non-live), 10% 환각(hallucination). 단일 턴은 풀렸다. 메모리, 동적 의사결정, 장기 지평(long-horizon) 도구 사슬은 아니다.
...
초기 도구 사용은 물었다: 모델이 올바른 함수 호출을 예측할 수 있는가? 현대의 도구 사용은 묻는다: 모델이 40단계에 걸쳐, 메모리를 가지고, 부분 관측가능성(partial observability)으로, 도구 실패에서 회복하면서, 존재하지 않는 도구를 환각하지 않고 도구를 연쇄할 수 있는가?
...
규모 결과: 도구 사용은 규모에서 창발한다. 작은 모델은 도구 주석에서 손해를 보고, 큰 모델은 이득을 본다. 이것이 2026년 프런티어 모델에 강한 도구 사용이 내장된 반면 대부분의 7B 모델은 신뢰성을 위해 명시적 도구 사용 파인튜닝이 필요한 이유다.
...
Anthropic은 `input_schema`를 직접 쓴다. OpenAI는 `function.parameters`를 쓴다. 둘 다 JSON Schema를 받는다. 설명(description)은 핵심을 떠받친다 — 모델은 그것을 읽고 올바른 도구를 고른다. 나쁜 도구 설명은 잘못된 도구 선택 실패의 1순위 근본 원인이다.
...
공학 규칙: 상관 ID를 핵심을 떠받치는 것으로 다뤄라. 그것을 뒤바꾸면 잘못된 도구-잘못된 결과 라우팅이 생긴다.
