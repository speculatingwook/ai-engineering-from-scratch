...
ReAct의 번갈아 도는 사고-행동-관찰 루프는 단순하고 유연하지만, 각 도구 호출은 이전의 모든 사고를 포함한 전체 사전 컨텍스트를 운반해야 한다. 토큰 사용량은 깊이에 따라 제곱으로 늘어난다. 더 나쁘게는, 도구가 루프 도중 실패하면 모델은 에러 관찰로부터 전체 계획을 다시 유도해야 한다.

ReWOO(Xu et al., arXiv:2305.18323, 2023년 5월)는 이를 알아채고 한 가지에 베팅했다: 전체를 앞쪽에서 계획하고, 증거를 병렬로 가져오고, 마지막에 답을 조합한다. 계획에 LLM 호출 한 번, 증거에 N번의 도구 호출(병렬 가능), 해결에 LLM 호출 한 번. 거래는 유연성을 덜 가지는 대신(계획이 정적임) 훨씬 나은 토큰 효율과 더 명확한 실패 양상을 얻는 것이다.
...
계획기는 DAG를 생성한다. 각 노드는 도구, 그 인자, 그리고 어느 앞선 노드에 의존하는지(예: `#E1`, `#E2` 같은 참조)를 명명한다. 작업자는 노드를 위상 정렬(topological) 순서로 실행한다. 해결기는 모든 것을 꿰맨다.
...
ReAct에서 작업자 3이 실패하면 루프는 스트림 도중에 에러를 빠져나오는 추론을 해야 한다. ReWOO에서는 작업자 3이 에러 문자열을 반환하고, 해결기는 그것을 원래 계획과 함께 컨텍스트에서 보고 우아하게 성능을 낮출 수 있다. 실패 국소화는 단계별이 아니라 노드별이다.
