...
CrewAI의 분할은 그 거래(trade)에 대해 정직하다. Crews는 협업적이고 역할 기반이며 탐색적인 작업을 위한 것. Flows는 이벤트 기반, 코드 소유, 감사 가능한 프로덕션을 위한 것. 같은 프레임워크, 두 형태, 표면(surface)마다 고른다.
...
- **Hierarchical.** 매니저(manager) Agent(별도의 LLM 호출)가 전문가들 사이를 라우팅한다. CrewAI는 당신의 `manager_llm` 설정 또는 기본값으로부터 매니저를 생성한다. 매니저는 매 라운드 다음 작업을 고르고 거부하거나 재라우팅할 수 있다. 네 명 이상의 전문가가 있고 순서가 진정으로 이전 출력에 의존할 때 사용한다.
...
- **Flow.** 당신이 소유하는 이벤트 기반 그래프. `@start`가 진입점을 표시한다. `@listen(topic)`이 다른 단계가 그 토픽을 방출할 때 발화하는 단계를 표시한다. 각 단계는 평범한 Python이다(내부적으로 Crew를 호출할 수 있다). 적합: 프로덕션. 관측 가능. 테스트 가능. 결정론적.
...
Crew에서 `memory=True` 또는 타입별 설정으로 활성화한다. 당신이 설정하는 임베딩(embedding) 제공자가 뒷받침한다(기본 OpenAI, 로컬로 교체 가능). 메모리는 CrewAI가 더 얇은 프레임워크에 맞서 값을 하는 곳 중 하나다. 순수 LangGraph는 이 각각을 직접 배선하도록 요구한다.
