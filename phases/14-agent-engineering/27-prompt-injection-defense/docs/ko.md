...
> Greshake et al.(AISec 2023)은 간접 프롬프트 인젝션(indirect prompt injection)을 에이전트(agent) 보안의 핵심 문제로 정립했다. 공격자는 에이전트가 검색하는 데이터에 지시를 심는다; 수집(ingest) 시점에 그 지시가 개발자 프롬프트(prompt)를 덮어쓴다. 검색된 모든 내용을 도구 사용(tool-use) 표면에서의 임의 코드 실행으로 취급하라.
...
- Bing Chat, GPT-4 코드 완성, 합성 에이전트에 대해 시연된 익스플로잇:
  ...
  - **지속적 메모리 오염(Persistent memory poisoning)** — 에이전트가 공격자의 지시를 저장한다; 다음 세션에서 자신을 재오염시킨다.
...
6. **외부 저장소를 통한 내용 포착.** Lesson 23 — 검색된 내용을 외부에 저장하라; 스팬(span)은 산문이 아니라 참조를 운반한다; 사고는 감사 가능하다.
...
- **Anthropic 도구 사용 모범 사례** — 검색된 내용을 신뢰할 수 없는 것으로 취급; Claude의 시스템 프롬프트가 이를 명시적으로 다룬다.
- **맞춤 PVE** — 도메인 특화 인젝션 패턴을 위한 당신만의 검증자 모델.
...
4. Greshake et al.을 처음부터 끝까지 읽어라. 시연된 익스플로잇 중 하나를 당신의 장난감에 구현하라. 그것을 고쳐라.
