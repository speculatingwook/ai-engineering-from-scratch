...
에이전트는 경계를 넘쳐 흐른다. 작업은 "로그인 버그를 고쳐라"이다. 그런데 디프는 로그인 라우트, 이메일 헬퍼, 데이터베이스 드라이버, README, 그리고 릴리스 스크립트를 건드린다. 각 변경에는 그 순간 그럴듯한 이유가 있었다. 하지만 그것들을 합치면, 검토받았던 변경과는 다른 변경이 된다.
...
실제 리포지터리는 파일을 이동시킨다. 계약을 글롭(`app/**/*.py`, `tests/test_signup*.py`)에 고정시켜서, 세션 사이의 리팩터링이 계약을 무효화하지 않도록 하라.
...
**이진 실패가 아니라 위반 예산(violation budget).** `agent-guardrails`(Claude Code, Cursor, Windsurf, Codex가 MCP를 통해 사용하는 OSS 머지 게이트)는 작업별 `violationBudget`을 제공한다. 예산 안에 드는 사소한 스코프 일탈은 경고로 표면화되고, 예산을 초과할 때만 머지 게이트가 거부한다. `violationSeverity: "error" | "warning"`과 짝지어라. 예산이야말로, 출시되는 게이트와 그것을 싫어하는 팀에 의해 비활성화되는 게이트 사이의 차이다.
...
**파일 예산 옆의 시간 예산과 네트워크 예산.** `time_budget_minutes` 필드는 벽시계 시간(wall clock)을 제한한다. 런타임은 재승인 없이 그 시간을 넘겨 계속하기를 거부한다. 호스트명에 대한 `network_egress` 허용 목록은 에이전트가 작업의 일부가 아니었던 외부 API를 조용히 호출하는 것을 막는다. 이것들도 스코프 차원(dimension)이다. 파일 글롭은 필요하지만 충분하지는 않다.
