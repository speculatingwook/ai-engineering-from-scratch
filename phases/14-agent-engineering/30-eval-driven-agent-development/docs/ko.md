...
이것은 Self-Refine(Lesson 05)를 일반화한 것이다. 당신이 신경 쓰는 어떤 에이전트 흐름이든 신뢰성을 위해 평가자-최적화자로 감쌀 수 있다.
...
1. 당신의 프로덕션 실패 중 하나를 골라라. 그것을 재현하는 평가 케이스를 작성하라. 당신의 에이전트가 지금 그것을 통과하는가?
2. 세 가지 차원(사실성, 톤, 범위)을 가진 당신 도메인용 LLM 판정 루브릭(rubric)을 만들어라. 50개 세션을 채점하라.
3. 평가 스위트를 CI에 연결하라. 5% 이상 회귀 시 빌드를 실패시켜라.
4. 궤적 효율성(trajectory-efficiency) 지표를 추가하라: 에이전트가 골드 궤적 대비 몇 스텝을 거쳤는가?
5. Phase 14의 모든 레슨을 당신 스위트의 평가 케이스에 매핑하라. 빠진 것이 있는가? 그것이 메워야 할 공백이다.
...
| 커스텀 오프라인 평가(Custom offline eval) | "도메인 평가" | 당신 제품 형태에 대한 LLM-as-judge / 실행 / 궤적 |
