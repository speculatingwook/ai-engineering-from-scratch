...
팀은 트레이스의 90%에서 작동하는 에이전트(agent)를 출하한다. 10%의 실패는 무작위 잡음이 아니다 — 소수의 반복되는 범주로 떨어진다. 일단 그것들을 이름 붙일 수 있으면, 그것들을 모니터링하고 고칠 수 있다.
...
연쇄가 치명적이다. 에이전트는 "내가 실패했다"와 "과제가 불가능하다"를 구별하지 못하며, 종종 400 오류에서 성공 메시지를 환각하여 루프를 닫는다.
...
- 당신의 관측성 플랫폼이 탐지할 수 없는 도메인 특화 시그니처에는 **맞춤(custom)**.

## 산출물 (Ship It)

`outputs/skill-failure-detector.md`는 트레이스 저장소에 배선된, 당신의 도메인에 맞춤화된 실패 양상 탐지기를 생성한다.

## 연습 문제 (Exercises)

1. "성공 환각" 탐지기를 추가하라: 에이전트가 성공을 반환하지만 대상 상태가 변하지 않음.
2. 당신이 만든 제품에서 실제 트레이스 100개를 태그하라. 어느 양상이 지배적인가? 그것을 고치는 비용은?
3. "연쇄 반경(cascade radius)" 지표를 구현하라: N 단계에서의 실패가 주어졌을 때, 몇 개의 하류 단계에 영향을 미쳤는가?
4. MASFT의 14가지 실패 양상을 읽어라. 당신의 제품에 적용되는 세 가지를 골라라. 탐지기를 작성하라.
