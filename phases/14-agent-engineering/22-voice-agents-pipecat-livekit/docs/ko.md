...
음성 에이전트는 TTS를 덧붙인 텍스트 루프가 아니다. 지연 시간 예산이 가혹하고(~600ms), 부분 오디오가 기본값이며, 턴 감지(turn detection)는 하나의 모델이고, 트랜스포트는 전화망 SIP부터 WebRTC까지 다양하다. 당신은 프레임 기반 파이프라인을 구축하거나(Pipecat) 플랫폼에 기대거나(LiveKit) 둘 중 하나다.
...
- **STT 신뢰도 무시.** 낮은 신뢰도의 전사(transcript)가 마치 진리인 듯 LLM에 공급된다. 신뢰도로 게이트하거나 확인을 요청하라.
- **TTS 문장 중간 끊김.** 파이프라인이 발화 중간에 취소될 때, TTS는 이를 알거나 오디오를 끊어야 한다.
- **지연 시간 예산 무시.** 모든 구성요소가 50~200ms를 더한다. 출하 전에 당신의 체인을 합산하라.
...
1. 당신의 장난감 파이프라인에 지표 옵저버를 추가하라: 초당 단계별 프레임 수를 세어라. 지연 시간이 어디에 축적되는가?
