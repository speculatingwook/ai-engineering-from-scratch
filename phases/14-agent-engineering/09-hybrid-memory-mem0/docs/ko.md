...
Letta의 무효화 패턴을 일반화한 규정 준수 등급의 동작이다.
...
비교 베이스라인(전체 컨텍스트 128k LLM, 평평한 벡터 저장소, 평평한 KV)은 모두 10점 이상 차이로 진다. 선택을 정당화하는 것은 벤치마크가 아니라 운영 형태지만, 그래도 이 수치는 융합 설계가 반올림 오차가 아님을 보여준다.
