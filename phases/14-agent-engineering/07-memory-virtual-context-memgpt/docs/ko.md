...
더 큰 윈도우는 도움이 되지만 이를 고치지 못한다. Mem0의 2025년 논문은 128k 윈도우 베이스라인(baseline)이 외부 메모리를 가진 4k 윈도우 에이전트가 잡는 장기 지평(long-horizon) 사실을 여전히 놓친다고 측정했다.
...
에이전트는 정상적인 ReAct 루프를 돌린다. 한 가지 추가 도구 부류가 데이터를 메인 컨텍스트로 페이지 인/아웃하게 한다.
