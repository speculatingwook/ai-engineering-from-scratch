...
> 열한 개 레슨 분량의 표면(surface)도 실제 코드베이스와의 접촉에서 살아남지 못한다면 아무 가치가 없다. 이 레슨은 작은 샘플 앱에서 같은 작업을 두 번 실행한다: 프롬프트 전용(prompt-only) 대 워크벤치 안내(workbench-guided). 숫자가 논쟁을 한다.
...
장난감 작업에 대한 데모는 아무도 설득하지 못한다. 워크벤치에 대한 근거는, 실제처럼 느껴지는 리포지터리(repo)의 실제처럼 느껴지는 작업이 더 적은 실패, 더 적은 되돌림(revert), 그리고 다음 세션이 사용할 수 있는 패킷(packet)과 함께 프로덕션(production)에 안착할 때 만들어진다.
...
회의론자의 질문은 "워크벤치가 실제로 얼마나 도움이 되는가?"이다. 2026년 숫자는 설명보다 훨씬 더 많은 것을 말한다.
...
**기업 AI 에이전트 프로젝트의 88%가 프로덕션에 도달하지 못한다.** preprints.org *Harness Engineering for Language Agents* 논문(2026년 3월)은 그 실패를 추론(reasoning)이 아니라 런타임으로 추적한다: 오래된 상태(stale state), 깨지기 쉬운 재시도(brittle retries), 과도하게 자란 컨텍스트, 중간 실수로부터의 빈약한 복구.
...
2. 당신 코드베이스의 실제 둘째 날 작업에 대해 비교를 실행하라. 워크벤치 숫자가 어디서 미끄러지는가?
...
| 워크벤치 벤치마크 (Workbench benchmark) | "신뢰성 점수" | 당신의 코드베이스에서 비교를 실행하는 이식 가능한 하니스 |
