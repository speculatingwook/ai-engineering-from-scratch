...
각 노드는 일관된 중간 단계("하나의 사고")다. 각 노드는 K개의 자식 사고로 확장될 수 있다. LLM은 채점 프롬프트로 각 노드를 자기 평가한다. 탐색은 트리를 탐험한다 — BFS, DFS, 또는 빔(beam).
...
- **자기 성찰자(Self-reflector)**: 실패 시 자연어 성찰(Reflexion 스타일)을 쓰고 그것을 미래 롤아웃(rollout)을 다시 씨 뿌리는 데 사용.

환경 피드백(관찰)이 가치 함수에 섞여 들어가, 탐색이 모델 의견만이 아니라 실제 도구 결과로 정보를 얻는다. 논문 시점 결과: GPT-4로 HumanEval pass@1 92.7%(SOTA), GPT-3.5로 WebShop 평균 75.9(그래디언트 기반 파인튜닝(fine-tuning)에 근접).
...
탐색은 토큰을 폭발시킨다. Game of 24에서 ToT는 CoT 토큰의 100~1000배를 쓴다. LATS도 비슷하다. 이는 공짜가 아니다. 탐색은 다음을 위해 아껴라:
...
작업에 정답이 하나뿐이고 평가자가 시끄럽다면, 탐색은 종종 상황을 악화시킨다 — "점수 좋은" 틀린 답을 찾아낸다.

### 2026년 위치

대부분의 프로덕션 에이전트는 LATS를 돌리지 않는다. 도구에 근거한 검증(CRITIC, Lesson 05)을 갖춘 ReAct를 돌린다. 탐색은 특수한 틈새에서 나타난다:
