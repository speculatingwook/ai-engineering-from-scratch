...
> 세션은 끝날 것이다. 작업은 아니다. 핸드오프 패킷(handoff packet)은 "에이전트가 한 시간 동안 일했다"를 "다음 세션이 첫 1분 만에 생산적이다"로 바꾸는 아티팩트(artifact)다. 사후 생각이 아니라 의도적으로 만들어라.
...
세션이 끝난다. 에이전트(agent)는 "좋아요, 우리는 진전을 이뤘습니다"라고 말한다. 다음 세션이 열린다. 다음 에이전트는 "우리가 어디서 멈췄죠?"라고 묻는다. 첫 번째 에이전트의 답은 사라졌다. 다음 에이전트는 재발견하고, 같은 명령을 다시 실행하고, 사람에게 같은 질문을 다시 묻고, 이전 세션의 마지막 30초를 복구하느라 30분을 태운다.
...
Codex CLI, Claude Code, OpenCode는 각각 다른 압축(compaction) 이야기를 제공한다. 구조화된 핸드오프 패킷은 셋 모두의 위에 앉는다.
