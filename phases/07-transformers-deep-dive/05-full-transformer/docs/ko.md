...
단일 어텐션 층(layer)은 모델이 아니라 특성(feature) 추출기다. 층당 하나의 행렬곱(matmul)은 언어를 다루기에 충분한 용량이 아니다. 깊이가 필요하다 — 그리고 깊이는 올바른 배관(plumbing) 없이는 무너진다.
...
원 논문: `x + sublayer(LN(x))` vs `LN(x + sublayer(x))`. Post-norm은 2019년경 인기를 잃었다 — 세심한 워밍업 없이 깊게 학습하기가 더 어렵다. Pre-norm(하위 층 *전*에 `LN`)이 2026년 기본값이다. Llama, Qwen, GPT-3+, Mistral 모두 이를 쓴다.
...
RMSNorm은 LayerNorm의 평균 중심화를 버린다(뺄셈 하나 줄임). 이는 계산을 아끼고 경험적으로 최소한 동등하게 안정적이다. SwiGLU(`Swish(W1 x) ⊙ W3 x`)는 Llama, PaLM, Qwen 논문에서 ReLU/GELU FFN을 일관되게 약 0.5포인트 ppl 능가한다.
