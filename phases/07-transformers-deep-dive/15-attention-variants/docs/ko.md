...
희소 어텐션은 커널 엔지니어링 이야기다. 수학은 단순하다(점수 행렬을 마스킹(mask)). 이득은 0 항목을 절대 SRAM으로 로드하지 않는 데서 나온다. FlashAttention-3와 2026년의 FlexAttention API는 커스텀 희소 패턴을 PyTorch에서 일급(first-class)으로 만든다.
