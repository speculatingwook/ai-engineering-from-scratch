...
당신에게 데이터셋(dataset)이 있다. 새 데이터 포인트가 도착한다. 당신은 그것을 분류하거나 그 값을 예측해야 한다. 데이터로부터 파라미터(parameter)를 학습(선형 회귀나 SVM처럼)하는 대신, 그저 새 점에 가장 가까운 K개의 학습 점을 찾아 그것들이 투표하게 한다.

이것이 k-최근접 이웃(K-nearest neighbors, KNN)이다. 학습 단계가 없다. 학습할 파라미터가 없다. 최소화할 손실 함수가 없다. 전체 학습 셋을 저장하고 예측 시점에 거리를 계산한다.

너무 단순해서 동작하지 않을 것 같다. 하지만 KNN은 많은 문제, 특히 작은~중간 크기 데이터셋에서 놀랍도록 경쟁력이 있으며, 이를 깊이 이해하면 근본 개념들이 드러난다. 거리 지표의 선택(Phase 1 Lesson 14와 연결됨), 차원의 저주, 그리고 게으른(lazy) 학습과 부지런한(eager) 학습의 차이다.

KNN은 또한 현대 AI 어디에나 등장하는데, 단지 다른 이름으로다. 벡터 데이터베이스는 임베딩(embedding)에 대해 KNN 탐색을 한다. 검색 증강 생성(RAG)은 K개의 가장 가까운 문서 청크를 찾는다. 추천 시스템은 비슷한 사용자나 항목을 찾는다. 알고리즘은 같다. 규모와 자료 구조가 다를 뿐이다.
...
**문제 2: 부피가 폭발한다.** 데이터의 고정된 비율 안에서 K개 이웃을 포착하려면, 특성 공간의 훨씬 더 큰 비율을 덮도록 탐색 반경을 확장해야 한다. 고차원에서 "이웃"은 공간의 대부분을 포함한다.
...
최근접 이웃을 찾으려면, 질의를 포함하는 잎(leaf)까지 트리를 순회한 뒤, 역추적하면서 더 가까운 점을 포함할 수 있는 이웃 분할만 확인한다.
