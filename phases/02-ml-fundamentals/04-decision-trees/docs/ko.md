...
> 결정 트리(decision tree)는 그저 순서도일 뿐이다. 하지만 그것들의 숲(forest)은 ML에서 가장 강력한 도구 중 하나다.
...
당신에게 표 형태의 데이터가 있다. 행은 샘플, 열은 특성(feature)이고, 예측하고 싶은 타깃 열이 있다. 신경망(neural network)을 던져볼 수도 있다. 하지만 표 형태 데이터에서는 트리 기반 모델(결정 트리, 랜덤 포레스트, 그래디언트 부스티드 트리)이 일관되게 딥러닝(deep learning)을 능가한다. 구조화된 데이터에 대한 Kaggle 대회는 트랜스포머가 아니라 XGBoost와 LightGBM이 지배한다.

왜인가? 트리는 전처리 없이 혼합된 특성 유형(수치형과 범주형)을 처리한다. 특성 공학(feature engineering) 없이 비선형 관계를 처리한다. 해석 가능하다. 트리를 보면 어떤 예측이 왜 이루어졌는지 정확히 볼 수 있다. 그리고 많은 트리를 평균하는 랜덤 포레스트는 중간 크기 데이터셋(dataset)에서 과적합(overfitting)에 매우 강하다.

이 레슨은 재귀적 분할을 사용해 결정 트리를 밑바닥부터 만든 다음, 그 위에 랜덤 포레스트를 만든다. 분할 기준(지니 불순도, 엔트로피, 정보 이득) 뒤에 있는 수학을 구현하고, 약한 학습기(weak learner)들의 앙상블(ensemble)이 어떻게 강한 학습기가 되는지 이해하게 될 것이다.
...
각 내부 노드는 어떤 특성을 임계값(threshold)과 비교해 테스트한다. 각 잎(leaf) 노드는 예측을 한다. 새 데이터 포인트를 분류하려면, 루트에서 시작해 잎에 도달할 때까지 가지를 따라간다.

트리는 위에서 아래로, 각 노드에서 데이터를 가장 잘 분리하는 특성과 임계값을 고르며 만들어진다. "가장 잘"은 분할 기준(split criterion)으로 정의된다.

### 분할 기준: 불순도 측정

각 노드에는 샘플 집합이 있다. 우리는 결과로 나오는 자식 노드들이 가능한 한 "순수"하도록, 즉 각 자식이 대부분 하나의 클래스를 포함하도록 분할하고 싶다.
...
이 탐욕적 접근법은 전역적으로 최적인 트리를 보장하지 않는다. 최적 트리를 찾는 것은 NP-하드다. 하지만 탐욕적 분할은 실무에서 잘 동작한다.

### 정지 조건

정지 조건이 없으면, 트리는 모든 잎이 순수해질 때까지(잎당 샘플 하나) 자란다. 이는 학습 데이터를 완벽히 암기하고 일반화는 끔찍하게 한다.
...
사전 가지치기가 더 단순하고 빠르다. 사후 가지치기는 유용한 추가 분할로 이어질 수 있는 분할을 성급하게 멈추지 않기 때문에 종종 더 나은 트리를 만든다.
...
단일 결정 트리는 분산이 높다. 데이터의 작은 변화가 완전히 다른 트리를 만들 수 있다. 랜덤 포레스트(random forest)는 많은 트리를 평균하여 이를 해결한다.
...
핵심 통찰: 많은 비상관(decorrelated) 트리를 평균하면 편향을 늘리지 않으면서 분산을 줄인다. 개별 트리는 평범할 수 있다. 앙상블은 강하다.
...
이는 빠르지만(학습 중에 계산됨) 카디널리티가 높은 특성과 가능한 분할 지점이 많은 특성 쪽으로 편향된다.
...
신경망은 데이터가 공간적 또는 순차적 구조(이미지, 텍스트, 오디오)를 가질 때 이긴다. 특성들의 평평한 표에 대해서는 트리가 기본 선택이다.
...
실무에서 그래디언트 부스티드 트리(XGBoost, LightGBM, CatBoost)는 종종 랜덤 포레스트보다 강한데, 트리를 순차적으로 만들면서 각 트리가 이전 트리들의 오류를 교정하기 때문이다. 하지만 랜덤 포레스트는 잘못 구성하기가 더 어렵고 하이퍼파라미터 튜닝이 거의 필요 없다.

## 산출물 (Ship It)

이 레슨은 `outputs/prompt-tree-interpreter.md`를 만들어낸다 -- 비즈니스 이해관계자를 위해 결정 트리 분할을 해석하는 프롬프트(prompt)다. 학습된 트리의 구조(깊이, 특성, 분할 임계값, 정확도)를 넣으면, 모델을 평이한 언어의 규칙으로 번역하고, 특성 중요도 순위를 매기고, 과적합이나 누출(leakage)을 표시하고, 다음 단계를 권장한다. 코드를 읽지 않는 사람에게 트리 기반 모델을 설명해야 할 때 언제든 사용하라.
