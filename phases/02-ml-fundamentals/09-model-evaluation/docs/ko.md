...
- K-겹(K-fold)과 계층화 K-겹(stratified K-fold) 교차 검증(cross-validation)을 밑바닥부터 구현하고, 왜 불균형 데이터에 계층화가 중요한지 설명하기
- 정밀도(precision), 재현율(recall), F1, AUC-ROC, 그리고 회귀 지표(MSE, RMSE, MAE, R-제곱)를 밑바닥부터 계산하기
- 학습 곡선(learning curve)을 해석하여 모델이 높은 편향(high bias)에 시달리는지 높은 분산(high variance)에 시달리는지 진단하기
- 데이터 누출(data leakage), 잘못된 지표 선택, 테스트 셋 오염을 포함한 흔한 평가 실수를 식별하기

## 문제 (The Problem)

당신은 모델을 학습시켰다. 그것은 당신의 데이터에서 95% 정확도를 얻는다. 좋은가?

그럴 수도 있다. 아닐 수도 있다. 데이터의 95%가 한 클래스에 속한다면, 항상 그 클래스를 예측하는 모델은 완전히 쓸모없으면서도 95% 정확도를 얻는다. 학습한 데이터와 같은 데이터로 평가했다면, 모델이 답을 그냥 외운 것이므로 95%라는 숫자는 무의미하다. 데이터셋(dataset)에 시간 요소가 있는데 분할 전에 무작위로 섞었다면, 모델이 미래 데이터를 써서 과거를 예측하고 있을 수도 있다.

모델 평가는 대부분의 ML 프로젝트가 잘못되는 지점이다. 잘못된 지표는 나쁜 모델을 좋아 보이게 만든다. 잘못된 분할은 모델이 부정행위를 하게 한다. 잘못된 비교는 당신이 더 나쁜 모델을 고르게 만든다. 평가를 제대로 하는 것은 선택 사항이 아니다. 그것은 프로덕션(production)에서 동작하는 모델과 실제 데이터를 보는 순간 실패하는 모델 사이의 차이다.
...
- **검증 셋(Validation set)**: 하이퍼파라미터(hyperparameter)를 튜닝하고 모델들 사이에서 선택하는 데 쓰인다. 모델은 이 데이터로 결코 학습하지 않지만, 당신의 결정이 이것에 영향을 받는다.
- **테스트 셋(Test set)**: 최종 성능을 보고하기 위해 맨 마지막에 정확히 한 번 건드린다. 테스트 성능을 보고 나서 돌아가 모델을 바꾼다면, 그것은 더 이상 테스트 셋이 아니다. 두 번째 검증 셋이 되어버린 것이다.

테스트 셋은 보고된 성능이 모델이 정말로 보지 못한 데이터에서 어떻게 할지를 반영한다는, 당신의 홀드아웃(hold-out) 보증이다.

### K-겹 교차 검증

작은 데이터셋의 경우, 단일 학습/검증 분할은 데이터를 낭비하고 노이즈가 많은 추정을 준다. K-겹 교차 검증(K-fold cross-validation)은 모든 데이터를 학습과 검증 양쪽에 사용한다.
...
**계층화 K-겹(Stratified K-fold)**: 각 폴드에서 클래스 분포를 보존한다. 데이터셋이 70% 클래스 A, 30% 클래스 B라면, 각 폴드가 대략 같은 비율을 갖는다. 이는 무작위 분할이 모든 소수 샘플을 한 폴드에 넣을 수 있는 불균형 데이터셋에 중요하다.

### 분류 지표

**혼동 행렬(Confusion matrix)**: 토대다. 이진 분류(binary classification)의 경우:
...
이 행렬로부터 다른 모든 지표가 따라온다.

- **정확도(Accuracy)** = (TP + TN) / (TP + TN + FP + FN). 올바른 예측의 비율. 클래스가 불균형할 때 오해를 부른다.
- **정밀도(Precision)** = TP / (TP + FP). 양성으로 예측한 모든 것 중, 실제로 양성이었던 것은 얼마나 되는가? 거짓 양성이 비쌀 때 사용한다(예: 스팸 필터가 실제 이메일을 스팸으로 표시).
- **재현율(Recall)** (민감도, sensitivity) = TP / (TP + FN). 실제 양성 중, 우리가 잡아낸 것은 얼마나 되는가? 거짓 음성이 비쌀 때 사용한다(예: 암 검진이 종양을 놓침).
- **F1 점수(F1 score)** = 2 * precision * recall / (precision + recall). 정밀도와 재현율의 조화 평균. 어느 쪽도 명확히 지배하지 않을 때 둘의 균형을 맞춘다.
- **AUC-ROC**: 수신자 조작 특성(Receiver Operating Characteristic) 곡선 아래 면적. 다양한 분류 임계값(threshold)에서 참 양성률 대 거짓 양성률을 그린다. AUC = 0.5는 무작위 추측, AUC = 1.0은 완벽한 분리를 뜻한다. 임계값에 무관하다. 당신이 고른 컷오프와 상관없이 모델이 양성을 음성 위로 얼마나 잘 순위 매기는지 측정한다.
...
- **R-제곱(R-squared)** = 1 - SS_res / SS_tot, 여기서 SS_res = sum((y_true - y_pred)^2)이고 SS_tot = sum((y_true - y_mean)^2)이다. 모델이 설명하는 분산의 비율. R^2 = 1.0은 완벽. R^2 = 0.0은 모델이 항상 평균을 예측하는 것보다 나을 게 없음. R^2는 모델이 평균보다 나쁘면 음수가 될 수 있다.
...
- **높은 분산(과적합, overfitting)**: 학습 점수는 높지만 검증 점수가 훨씬 낮다. 둘 사이의 간극이 크다. 데이터를 더 추가하면 도움이 될 것이다.
...
**데이터 누출(Data leakage)**: 테스트 셋의 정보가 학습으로 새어 든다. 예시: 분할 전에 전체 데이터셋에 스케일러를 적합(fit)시키기, 시계열 예측에 미래 데이터를 포함하기, 타깃에서 파생된 특성을 사용하기. 항상 먼저 분할하고, 그다음 전처리하라.

**클래스 불균형(Class imbalance)**: 거래의 99%가 정상이고 1%가 사기다. 항상 "정상"을 예측하는 모델은 99% 정확도를 얻는다. 대신 정밀도, 재현율, F1, 또는 AUC-ROC를 사용하라.
...
scikit-learn으로는, 평가가 워크플로에 내장되어 있다.
...
밑바닥 버전들은 교차 검증이 정확히 무엇을 하는지(마법이 아니라, 그저 for-루프와 인덱스 추적), 각 지표가 어떻게 계산되는지(그저 TP/FP/TN/FN을 세는 것), 그리고 왜 계층화가 중요한지(각 폴드에서 클래스 비율을 보존하는 것)를 보여준다. 라이브러리 버전은 병렬성, 더 많은 채점 옵션, 그리고 파이프라인과의 통합을 더한다.
...
2. 중첩 교차 검증 루프를 만들어라. 외부 루프는 모델 성능을 평가하고, 내부 루프는 하이퍼파라미터를 튜닝한다. 검증 데이터를 평가로 누출하지 않고 두 모델을 공정하게 비교하는 데 사용하라.
3. 모델 비교를 위한 순열 검정(permutation test)을 구현하라. 레이블을 섞고, 재학습하고, 성능을 측정한다. 100번 반복하여 귀무 분포를 만든다. 이 분포에 대해 관측된 모델 성능의 p-값을 계산하라.
