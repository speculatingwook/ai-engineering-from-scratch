...
당신은 스팸 필터를 만들고 싶다. 전통적인 접근법은 이렇다. 앉아서 수백 개의 규칙을 작성한다. "이메일에 'FREE MONEY'가 들어 있으면 스팸으로 표시한다. 느낌표가 3개를 넘으면 스팸으로 표시한다." 몇 주에 걸쳐 규칙을 작성한다. 그러면 스패머들이 표현을 바꾼다. 당신의 규칙이 깨진다. 당신은 규칙을 더 작성한다. 이 순환은 끝나지 않는다.

머신러닝(machine learning)은 이를 뒤집는다. 규칙을 작성하는 대신, 컴퓨터에 레이블(label)이 붙은 이메일 수천 개("스팸" 또는 "스팸 아님")를 주고 규칙을 스스로 알아내게 한다. 컴퓨터는 당신이 결코 생각해 내지 못했을 패턴을 찾아낸다. 스패머들이 전술을 바꾸면, 코드를 다시 쓰는 대신 새 데이터로 재학습(retrain)한다.

이렇게 "규칙을 프로그래밍하는 것"에서 "데이터로부터 학습하는 것"으로의 전환이 머신러닝의 핵심이다. 모든 추천 엔진, 음성 비서, 자율주행차, 언어 모델이 이런 방식으로 동작한다.
...
전통적인 프로그래밍: 당신이 규칙을 작성한다. 프로그램은 그 규칙을 데이터에 적용하여 출력을 만든다.

머신러닝: 당신이 데이터와 기대 출력을 제공한다. 알고리즘이 규칙을 발견한다.
...
실무에서 당신이 만들게 될 것의 대부분은 지도 학습을 쓴다. 비지도 학습은 전처리와 탐색에 흔히 쓰인다. 강화 학습은 게임 AI, 로보틱스, 그리고 언어 모델을 위한 RLHF를 구동한다.
...
**정제 및 탐색(Clean & Explore)**: 결측값을 처리하고, 중복을 제거하고, 분포를 시각화하고, 이상치를 찾는다. 이 단계는 종종 전체 프로젝트 시간의 60~80%를 차지한다.
...
이것은 초보자가 가장 많이 틀리는 개념이다. 당신은 모델을 학습 중에 한 번도 본 적 없는 데이터로 평가해야 한다. 그렇지 않으면 학습이 아니라 암기를 측정하는 것이다.
...
테스트 셋은 신성하다. 정확히 한 번만 본다. 테스트 성능을 기준으로 모델을 계속 조정한다면, 사실상 테스트 셋으로 학습하는 것이며 당신이 보고하는 수치는 무의미해진다.
...
**분산(Variance)**: 학습 데이터의 작은 변동에 대한 민감성에서 오는 오차. 높은 분산을 가진 모델은 서로 다른 데이터 부분집합으로 학습했을 때 매우 다른 예측을 내놓는다. 높은 분산은 과적합으로 이어진다.
...
축소 불가능한 노이즈(irreducible noise)는 줄일 수 없다(데이터 자체에 있는 무작위성이다). 당신은 bias^2 + variance가 최소가 되는 최적점을 찾고 싶은 것이다.
...
모든 ML 모델은 사소한 베이스라인(baseline)과 비교해야 한다. 여기서 베이스라인은 무작위 클래스를 예측한다. 당신의 ML 모델이 무작위 추측을 이기지 못한다면, 뭔가 잘못된 것이다.
...
이러한 한계가 당신이 배우게 될 다른 모든 알고리즘의 동기가 된다. k-최근접 이웃(K-nearest neighbors)은 여러 군집을 처리한다. 결정 트리(decision tree)는 비선형 경계를 처리한다. 특성 스케일링(feature scaling)은 스케일 문제를 해결한다. 각 레슨은 이전 레슨의 한계 위에 쌓인다.
