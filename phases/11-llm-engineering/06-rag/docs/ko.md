...
> 당신의 LLM은 학습 마감 시점까지의 모든 것을 안다. 당신 회사의 문서, 당신의 코드베이스, 지난주 회의록에 대해서는 아무것도 모른다. RAG는 관련 문서를 검색해 프롬프트(prompt)에 쑤셔 넣음으로써 이를 해결한다. 그것은 프로덕션 AI에서 가장 많이 배포된 패턴이다. 이 강좌에서 한 가지를 만든다면, RAG 파이프라인(pipeline)을 만들어라.
...
당신은 회사를 위한 챗봇을 만든다. 한 고객이 "What's the refund policy for enterprise plans?"라고 묻는다. LLM은 전형적인 SaaS 환불 정책에 대한 일반적인 답으로 응답한다. 200페이지 내부 위키에 묻혀 있는 실제 정책은 엔터프라이즈 고객이 60일 기간에 일할 계산 환불을 받는다고 말한다. LLM은 이 문서를 본 적이 없다. 학습되지 않은 것을 알 수는 없다.

파인튜닝은 한 가지 해법이다. LLM을 가져와, 내부 문서로 학습시키고, 갱신된 모델을 배포한다. 이것은 작동하지만 심각한 문제가 있다. 파인튜닝은 수천 달러의 연산 비용이 든다. 문서가 바뀌는 순간 모델은 낡는다. 모델이 어느 출처에서 끌어왔는지 알 방법이 없다. 그리고 회사가 다음 달에 다른 제품 라인을 인수하면, 다시 파인튜닝한다.

RAG는 다른 해법이다. 모델은 건드리지 않는다. 질문이 들어오면, 문서 저장소에서 관련 구절을 검색하고, 질문 앞에 프롬프트로 붙여 넣고, 모델이 그 구절들을 맥락으로 사용해 답하게 한다. 문서 저장소는 몇 분 안에 갱신할 수 있다. 어느 문서가 검색되었는지 정확히 볼 수 있다. 모델 자체는 절대 바뀌지 않는다. 이것이 RAG가 프로덕션에서 지배적인 패턴인 이유다: 더 저렴하고, 더 신선하고, 더 감사 가능하며, 어느 LLM과도 작동한다.
...
| 데이터 프라이버시 | 학습 데이터가 가중치에 구워짐 | 문서가 당신의 벡터 저장소에 남음 |

파인튜닝은 모델의 가중치(weight)를 영구적으로 바꾼다. RAG는 모델의 컨텍스트를 일시적으로 바꾼다. 대부분의 애플리케이션에서, 일시적 컨텍스트가 당신이 원하는 것이다.

파인튜닝이 이기는 한 가지 경우: 프롬프팅만으로는 달성할 수 없는 특정 스타일, 어조, 추론 패턴을 모델이 채택해야 할 때. 사실 지식 검색에 대해서는, RAG가 매번 이긴다.
...
임베딩을 갖게 되면, 그것들을 저장하고 검색할 어딘가가 필요하다. 옵션:
