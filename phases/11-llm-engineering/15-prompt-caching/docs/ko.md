...
> 당신의 시스템 프롬프트(prompt)는 4,000토큰(token)이다. 당신의 RAG 컨텍스트는 20,000토큰이다. 당신은 모든 요청에 둘 다 보낸다. 그리고 둘 다에 비용을 낸다 — 매번. 프롬프트 캐싱(prompt caching)은 프로바이더가 그 프리픽스를 자기 쪽에서 따뜻하게 유지하고 재사용 시 정상 요율의 10%만 청구하게 해준다. 올바르게 사용하면, 추론(inference) 비용을 50–90% 줄이고 첫 토큰 지연 시간(latency)을 40–85% 줄인다.
...
코딩 에이전트(agent)가 대화의 모든 턴마다 동일한 15,000토큰 시스템 프롬프트를 Claude에 보낸다. 입력 토큰 100만당 $3로 20턴이면 입력 비용만 $0.90다 — 사용자의 실제 메시지 어느 것도 들어가기 전에. 하루 10,000개의 대화를 곱하면 청구서가 절대 변하지 않는 텍스트에 대해 하루 $9,000에 도달한다.

품질을 해치지 않고는 프롬프트를 줄일 수 없다. 보내지 않을 수도 없다 — 모델은 모든 턴에 그것이 필요하다. 유일한 수는 프로바이더가 이미 본 프리픽스에 대해 정가를 지불하는 것을 멈추는 것이다.

그 수가 프롬프트 캐싱이다. Anthropic이 2024년 8월에 출시했고(2025년에 1시간 확장 TTL 변형과 함께), OpenAI가 그해 후반에 자동화했고, Google이 Gemini 1.5와 함께 명시적 컨텍스트 캐싱을 출시했고, 이제 세 곳 모두 자신들의 프런티어 모델에서 이를 일급(first-class) 기능으로 제공한다.

## 개념 (The Concept)

![Prompt caching: write once, read cheap](../assets/prompt-caching.svg)

**메커니즘.** 요청의 프리픽스가 최근 요청의 것과 일치하면, 프로바이더는 토큰을 다시 인코딩하는 대신 이전 실행의 KV 캐시를 제공한다. 당신은 첫 번째에 작은 쓰기 프리미엄을 지불하고 그 이후 매번 큰 읽기 할인을 받는다.
...
**불변 규칙.** 셋 모두 프리픽스만 캐시한다. 요청 간에 어떤 토큰이든 다르면, 첫 번째로 다른 토큰 이후의 모든 것이 미스다. *안정적인* 부분을 위에, *가변적인* 부분을 아래에 두라.
...
Anthropic의 25% 쓰기 프리미엄은 캐시된 블록이 순(net) 절약을 하려면 최소 두 번 읽혀야 함을 의미한다. 쓰기 1 + 읽기 1은 요청당 평균 0.675배 비용이다(32% 절약); 쓰기 1 + 읽기 10은 평균 0.205배다(80% 절약). 경험칙: TTL 내에 최소 3회 재사용할 것으로 예상되는 것은 무엇이든 캐시하라.
...
`cache_control` 마커는 Anthropic에게 그 블록을 5분간 저장하라고 알린다. 그 윈도우 내의 재사용은 히트한다; 만료 후의 재사용은 다시 쓴다.
...
CI에서 두 필드를 모두 확인하라 — `cache_read_input_tokens`가 요청 전반에 걸쳐 0에 머무르면, 당신의 캐시 키가 드리프트하고 있는 것이다.

### 2단계: 1시간 확장 TTL

장시간 실행되는 배치(batch) 작업의 경우, 5분 기본값은 작업 사이에 만료된다. `ttl`을 설정하라:

```python
{"type": "text", "text": RUBRIC, "cache_control": {"type": "ephemeral", "ttl": "1h"}}
```

1시간 TTL은 쓰기 프리미엄의 2배(25% 대신 베이스라인 대비 50%)가 들지만 프리픽스를 5회 이상 재사용하는 어떤 배치에서든 빠르게 본전을 뽑는다.

### 3단계: OpenAI 자동 캐싱

OpenAI는 설정할 것을 주지 않는다. 최근 요청과 일치하는 1,024토큰 이상의 어떤 프리픽스든 자동으로 50% 할인을 받는다.
...
동일한 캐시 친화적 레이아웃 규칙이 적용된다. Anthropic의 캐시는 죽이지 않지만 OpenAI의 캐시를 죽이는 두 가지: `user` 필드 변경(캐시 키 구성 요소로 사용됨)과 도구 재정렬.

### 4단계: Gemini 명시적 컨텍스트 캐싱

Gemini는 캐시를 당신이 생성하고 이름 붙이는 일급 객체로 취급한다:
...
Gemini는 캐시가 살아 있는 한 토큰·시간당 저장소 비용을 청구하고, 정상 입력 요율의 ~25%로 읽는다. 이것은 여러 날에 걸쳐 많은 세션에서 같은 거대한 프롬프트를 재사용할 때 올바른 형태다.

### 5단계: 프로덕션에서 히트율 측정하기

쓰기/읽기/미스 횟수를 추적하고 1K 요청당 혼합 비용을 계산하는 시뮬레이션된 3-프로바이더 회계기는 `code/main.py`를 보라. 목표 히트율에 배포를 게이팅하라 — 대부분의 프로덕션 Anthropic 설정은 워밍업 후 >80% 읽기 비율을 봐야 한다.
...
- **자유 텍스트 근접 중복.** "You are helpful." 대 "You are a helpful assistant." — 1바이트 차이 = 완전한 미스.
- **너무 작은 블록.** Anthropic은 1,024토큰 하한(Haiku는 2,048)을 강제한다. 더 작은 블록은 조용히 캐시되지 않는다.
- **눈먼 비용 대시보드.** "입력 토큰"을 캐시됨 대 캐시 안 됨으로 분리하라. 그렇지 않으면 트래픽 감소가 캐시 승리처럼 보인다.
...
사용자 메시지 계층을 위해 시맨틱 캐싱(Phase 11 · 11)과 결합하라: 프롬프트 캐싱은 *토큰 동일* 재사용을 처리하고, 시맨틱 캐싱은 *의미 동일* 재사용을 처리한다.
