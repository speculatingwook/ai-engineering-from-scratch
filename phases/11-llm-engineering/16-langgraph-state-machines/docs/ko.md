...
당신은 함수 호출(function calling) 에이전트를 출시한다. 세 턴 동안 동작하다가, 무언가 잘못된다: 모델이 500을 반환하는 도구를 시도하거나, 사용자가 작업 도중에 마음을 바꾸거나, 에이전트가 사람의 승인 없이 주문을 환불하기로 결정한다. `while True:` 루프에는 후크가 없다. 그것을 일시 정지할 수 없고, 되감을 수 없고, "모델이 다른 도구를 골랐다면 어땠을까"로 분기할 수 없다. 이것을 데모 너머로 출시하는 순간, 에이전트는 동작했거나 안 했거나인 블랙박스가 된다.
...
LangGraph는 이 추상화를 출시하는 라이브러리다. 그것은 LangChain적 의미의 에이전트 프레임워크("여기 AgentExecutor가 있으니, 행운을 빈다")가 아니다. 그것은 일급(first-class) 상태, 일급 영속성, 일급 인터럽트를 가진 그래프 런타임이다. 에이전트 루프는 당신이 손으로 작성하는 것이 아니라 그리는 것이다.
...
당신은 그래프를 컴파일한다. 컴파일은 토폴로지를 바인딩하고, 체크포인터(선택적이지만 프로덕션에는 필수)를 부착하고, 실행 가능한 것을 반환한다. 당신은 초기 상태와 `thread_id`로 그것을 호출한다. 실행의 모든 단계는 `(thread_id, checkpoint_id)`로 키가 매겨진 체크포인트를 영속화한다.
...
**인터럽트.** 노드를 `interrupt_before=["human_review"]`로 표시하면 그 노드가 실행되기 전에 실행이 멈춘다. 상태가 영속화된다. 당신의 API는 "승인 대기 중"으로 사용자에게 응답한다. 같은 `thread_id`에 대한 나중의 요청이 `Command(resume=...)`로 실행을 재개한다.

**스트리밍.** `graph.stream(state, mode="updates")`는 상태 델타가 발생하는 대로 산출한다. `mode="messages"`는 모델 노드 내부의 LLM 토큰을 스트리밍한다. `mode="values"`는 전체 스냅샷을 산출한다. 당신은 UI에 무엇을 표시할지 고른다.
...
모든 상태 필드는 리듀서를 가진다. 대부분의 기본값은 괜찮다 — 새 값이 옛 값을 덮어쓴다. 하지만 메시지 리스트는 새 메시지가 교체 대신 추가되도록 `operator.add`가 필요하다. 병렬 간선은 리듀서를 통해 그들의 업데이트를 병합한다. 두 노드가 모두 `messages`를 업데이트하는데 `Annotated[list, add_messages]`를 잊었다면, 두 번째가 조용히 이기고 당신은 턴의 절반을 잃는다. 리듀서는 라이브러리에서 유일하게 미묘한 것이다; 그것을 제대로 하면 나머지는 조합된다.
...
그게 전부다. 약 40줄의 코드로 체크포인팅, 인터럽트, 스트리밍을 갖춘 전체 ReAct 루프(Thought → Action → Observation → Thought → …)를 얻는다.

### StateGraph 대 Send (팬아웃)

`Send(node_name, state)`는 노드가 병렬 서브그래프를 디스패치하게 한다. 예시: 에이전트가 세 개의 검색기를 동시에 쿼리하기로 결정한다. 각 `Send`는 대상 노드의 병렬 실행을 생성한다; 그들의 출력은 상태 리듀서를 통해 병합된다. 이것이 LangGraph가 스레딩 프리미티브 없이 오케스트레이터-워커 패턴을 표현하는 방식이다.

### 서브그래프 (Subgraphs)

컴파일된 그래프는 다른 그래프의 노드가 될 수 있다. 외부 그래프는 단일 노드를 본다; 내부 그래프는 자체 상태와 자체 체크포인트를 가진다. 이것이 팀들이 슈퍼바이저-워커 에이전트를 구축하는 방식이다: 슈퍼바이저 그래프가 사용자 의도를 도메인별 워커 서브그래프로 라우팅한다.
...
모든 업데이트는 dict `{node_name: state_delta}`다. 당신의 프런트엔드는 이것들을 UI로 스트리밍하여 사용자가 "에이전트가 생각 중… search_web 호출 중… 결과 받음… 답변 중"을 보게 할 수 있다.
...
입력으로 `None`을 전달하면 주어진 체크포인트에서 재생한다; 값을 전달하면 재개 전에 그 체크포인트 상태에 대한 업데이트로 추가한다. 이것이 전체 대화를 다시 실행하지 않고 나쁜 에이전트 실행을 재현하는 방식이다.
...
> 당신은 에이전트를 `while True` 루프가 아니라 그래프로 구축한다.

LangGraph에 손을 뻗기 전에, 60초 설계를 하라:

1. **노드 이름 짓기.** 모든 별개의 결정이나 부수 효과(side effect)가 있는 액션은 노드다. "에이전트가 생각한다," "도구가 실행된다," "리뷰어가 승인한다," "응답이 스트리밍된다." 그것들을 나열할 수 없다면, 작업이 아직 에이전트 형태가 아니다.
2. **상태 선언하기.** 모든 리스트 필드에 리듀서를 가진 최소 TypedDict. 모든 것을 `messages`에 쑤셔넣지 마라; 작업별 필드(작업 중인 `plan`, `budget` 카운터, `retrieved_docs` 리스트)를 최상위로 끌어올려라.
...
5. **인터럽트를 도구 실행 후가 아니라 전에 결정하기.** 승인은 해를 끼치기 전에 취소할 수 있도록 부수 효과가 있는 노드로 들어가는 간선에 둔다; 검증은 나쁜 호출을 싸게 거부할 수 있도록 모델에서 나가는 간선에 둔다.
