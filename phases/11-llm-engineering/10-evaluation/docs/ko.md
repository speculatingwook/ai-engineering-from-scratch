...
> 당신은 테스트 없이 웹 앱을 배포(deployment)하지 않을 것이다. 롤백 계획 없이 데이터베이스 마이그레이션을 출시하지 않을 것이다. 하지만 지금 대부분의 팀은 출력 10개를 읽어보고 "음, 괜찮아 보이네"라고 말하면서 LLM 애플리케이션을 출시한다. 그것은 평가(evaluation)가 아니다. 그것은 희망이다. 희망은 엔지니어링 관행이 아니다. 모든 프롬프트(prompt) 변경, 모든 모델(model) 교체, 모든 temperature 조정은 몇 개의 예시를 읽어서는 예측할 수 없는 방식으로 출력 분포를 바꾼다. 평가는 당신의 애플리케이션과 조용한 품질 저하 사이를 가로막는 유일한 것이다.
...
- 당신의 LLM 애플리케이션에 특화된 입력-출력 쌍, 루브릭(rubric), 엣지 케이스를 갖춘 평가 데이터셋(dataset) 구축하기
- LLM-as-judge, 정규식 매칭, 결정론적 단언(assertion) 검사를 사용한 자동 채점 구현하기
- 프롬프트, 모델, 파라미터(parameter)가 변경될 때 품질 저하를 감지하는 회귀 테스트 설정하기
- 당신의 사용 사례에 중요한 것(정확성, 톤, 형식 준수, 지연 시간(latency))을 포착하는 평가 지표 설계하기

## 문제 (The Problem)

당신은 고객 지원을 위한 RAG 챗봇을 만든다. 데모에서는 훌륭하게 동작한다. 출시한다. 2주 후, 누군가 환각(hallucination)을 줄이기 위해 시스템 프롬프트를 변경한다. 그 변경은 효과가 있다 -- 환각률이 떨어진다. 하지만 답변 완전성도 34% 떨어진다. 모델이 이제 100% 확신하지 못하는 것에 대해서는 답변을 거부하기 때문이다.

11일 동안 아무도 알아채지 못한다. 셀프서비스 채널의 매출이 떨어진다. 지원 티켓이 급증한다.

이것이 감(vibe)으로 평가할 때의 기본 결과다. 몇 개의 예시를 확인하고, 괜찮아 보이면, 머지한다. 하지만 LLM 출력은 확률적(stochastic)이다. 테스트 케이스 5개에서 동작하는 프롬프트가 6번째에서는 실패할 수 있다. 당신의 벤치마크(benchmark)에서 92%를 받는 모델이 사용자가 실제로 부딪히는 엣지 케이스에서는 71%를 받을 수 있다.
...
**인간 평가(Human evaluation)**는 골드 스탠더드이지만 가장 느리고 가장 비싸다. 모든 커밋마다 실행하기 위해서가 아니라, 자동 평가를 보정하기 위해 아껴 쓰라.
...
이것은 당신이 90%의 경우에 사용할 평가 방법이다. 패턴은 간단하다: 강력한 모델에게 입력, 출력, 선택적 참조 답변, 그리고 루브릭을 준다. 채점하도록 요청한다.
...
**Best-of-N**은 각 입력에 대해 N개의 출력을 생성하고 심판이 가장 좋은 것을 고르게 한다. 이는 당신 시스템의 상한선을 측정한다. best-of-5가 일관되게 best-of-1을 이긴다면, 여러 응답을 샘플링(sampling)하고 선택하는 것이 이득일 수 있다.
...
당신의 평가 데이터셋은 그 안에 있는 케이스만큼만 좋다. 세 종류의 테스트 케이스가 중요하다:

**골든 테스트 세트(Golden test set)** (50-100 케이스): 당신의 핵심 사용 사례를 대표하는 큐레이션된 입력-출력 쌍. 이것들이 당신의 회귀 테스트다. 모든 프롬프트 변경은 이것들을 통과해야 한다.

**적대적 예시(Adversarial examples)** (20-50 케이스): 당신의 시스템을 깨뜨리도록 설계된 입력. 프롬프트 인젝션, 엣지 케이스, 모호한 쿼리, 당신의 도메인 밖 주제에 대한 질문, 유해한 콘텐츠 요청.

**분포 샘플(Distribution samples)** (100-200 케이스): 실제 프로덕션(production) 트래픽에서 무작위로 추출한 샘플. 이것들은 큐레이션된 테스트가 놓치는 문제를 잡아낸다. 사용자가 실제로 묻는 것을 반영하기 때문이다.

### 표본 크기와 신뢰도 (Sample Size and Confidence)

테스트 케이스 50개는 충분하지 않다.

당신의 평가가 50 케이스에서 90%를 받으면, 95% 신뢰 구간은 [78%, 97%]다. 이는 19포인트 범위다. 80%를 받는 시스템과 96%를 받는 시스템을 구별할 수 없다.
...
GPT-5-mini로 모든 PR마다 실행되는 200 케이스 평가 스위트는 실행당 ~$4가 든다. 당신의 팀이 주당 10개의 PR을 머지한다면, 그것은 월 $160다. 11일 동안 사용자 만족도를 떨어뜨리는 회귀를 출시하는 비용과 비교해 보라.

### 안티패턴 (Anti-Patterns)

**감 기반 평가.** "출력 5개를 읽었는데 좋아 보였어." 예시를 읽어서는 5% 품질 회귀를 지각할 수 없다. 당신의 뇌는 확증하는 증거만 골라낸다.

**학습 예시로 테스트하기.** 당신의 평가 케이스가 프롬프트나 파인튜닝(fine-tuning) 데이터의 예시와 겹친다면, 당신은 일반화가 아니라 암기를 측정하는 것이다. 평가 데이터를 분리해 두라.
...
**약한 심판 사용하기.** GPT-3.5를 심판으로 쓰면 잡음이 많고 일관성 없는 점수가 나온다. GPT-4o나 Claude Sonnet을 사용하라. 심판은 평가받는 모델만큼은 유능해야 한다.
...
이 레슨은 `outputs/prompt-eval-designer.md`를 만든다 -- 평가 루브릭을 설계하기 위한 재사용 가능한 프롬프트 템플릿이다. 당신의 LLM 애플리케이션에 대한 설명을 주면 고정된 채점 루브릭과 함께 맞춤형 평가 기준을 만들어 낸다.

또한 `outputs/skill-eval-patterns.md`를 만든다 -- 당신의 사용 사례, 예산, 품질 요구사항에 기반해 올바른 평가 전략을 선택하기 위한 결정 프레임워크다.
...
2. **쌍별 비교 만들기.** 심판을 수정해 두 모델 출력을 개별적으로 채점하는 대신 나란히 비교하게 하라. 동일한 입력과 두 출력이 주어지면, 심판은 어느 출력이 더 나은지와 그 이유를 반환해야 한다. baseline-v1 대 baseline-v2로 당신의 테스트 스위트 전체에서 쌍별 비교를 실행하고 신뢰 구간과 함께 승률을 계산하라.

3. **계층화 분석 구현하기.** 테스트 케이스를 카테고리(factual, technical, safety, coding, summarization)별로 그룹화하고 신뢰 구간과 함께 카테고리별 점수를 계산하라. 프롬프트 버전 간에 어느 카테고리가 개선되고 어느 카테고리가 회귀했는지 식별하라. 시스템은 전체적으로 개선되면서 특정 카테고리에서는 회귀할 수 있다.

4. **평가자 간 신뢰도 추가하기.** 각 테스트 케이스에 대해 LLM 심판을 3회 실행하라(서로 다른 심판 "평가자"를 시뮬레이션). 세 실행 사이의 코헨의 카파(Cohen's kappa) 또는 크리펜도르프의 알파(Krippendorff's alpha)를 계산하라. 합의가 0.7 미만이면 당신의 루브릭이 너무 모호한 것이다 -- 다시 작성하라.

5. **비용 추적기 만들기.** 모든 심판 호출의 토큰 사용량과 비용을 추적하라. 심판에 대한 각 입력은 원래 프롬프트, 모델 출력, 루브릭을 포함한다(입력 ~500 토큰, 출력 ~100 토큰). 당신의 테스트 스위트 전체에 걸친 총 평가 비용을 계산하고 주당 10회의 평가 실행을 가정해 월간 비용을 추정하라.
...
| 부트스트랩(Bootstrap) | "리샘플링" | 당신의 점수에서 복원 추출로 반복 샘플링해 신뢰 구간을 추정하는 것 -- 어떤 분포에서도 동작한다 |
