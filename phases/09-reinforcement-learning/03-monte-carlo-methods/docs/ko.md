...
당신에게는 환경으로부터 *샘플링*할 수 있는 능력만 필요한 방법이 필요하다. 정책을 돌린다. 궤적(trajectory) `s_0, a_0, r_1, s_1, a_1, r_2, …, s_T`를 얻는다. 그것을 사용해 가치를 추정한다. 그것이 몬테카를로다.
...
**이제 탐험이 문제다.** DP는 열거를 통해 모든 상태를 건드렸다. MC는 정책이 방문하는 상태만 본다. `π`가 결정론적이면, 상태 공간의 전체 영역이 결코 샘플링되지 않고, 그 가치 추정값은 영원히 0으로 남는다. 역사적 순서로 세 가지 해법:
...
3. **오프-폴리시(off-policy) MC.** 행동 정책(behavior policy) `μ` 하에서 데이터를 수집하고, 중요도 샘플링(importance sampling)을 통해 목표 정책 `π`에 대해 학습한다. 분산이 높지만, DQN 같은 리플레이 버퍼(replay-buffer) 방법으로 가는 다리다.
...
당신의 `V^π`에 대한 MC 추정값은 에피소드 → ∞에 따라 Lesson 02의 DP 결과와 일치해야 한다. 실전에서: 4×4 GridWorld에서 50,000 에피소드면 DP 답의 `~0.1` 이내에 든다.
...
현대 심층 강화 학습 알고리즘(PPO, SAC)은 `n`-스텝 리턴이나 GAE를 통해 순수 MC(전체 리턴)와 순수 TD(한-스텝 부트스트랩) 사이를 보간한다. 두 극단 모두 같은 추정기의 사례다.
