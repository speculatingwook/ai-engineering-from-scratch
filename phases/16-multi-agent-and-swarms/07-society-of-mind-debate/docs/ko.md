...
> 민스키(Minsky)의 1986년 전제 — 지능은 전문가들의 사회다 — 는 10년마다 재발견된다. 2023년 Du et al.은 이를 구체적인 알고리즘으로 바꿨다: 여러 LLM 인스턴스가 답을 제안하고, 서로의 답을 읽고, 비판하고, 갱신한다. N 라운드에 걸쳐 그들은 여섯 가지 추론 및 사실성 과제에서 제로샷 CoT와 리플렉션(reflection)을 이기는 합의(consensus)로 수렴한다. 두 가지 발견이 중요하다: **여러 에이전트(agent)**와 **여러 라운드**가 독립적으로 기여한다. 사회는 단일 에이전트의 독백을 이긴다; 다중 라운드 교환은 일회성 투표를 이긴다.
...
자기 일관성(self-consistency) — 한 모델을 여러 번 샘플링하고 다수 답을 취함 — 은 당신이 덧붙일 수 있는 가장 저렴한 추론 개선이다. 동작하지만, 빠르게 포화(saturation)된다. 샘플을 두 배로 늘려도 또 다른 의미 있는 도약을 보지 못할 수 있다.
