...
이는 고전적 멀티 에이전트 발견을 반영한다. 메커니즘을 통신 층에서 분리하는 것이 이긴다. Contract Net Protocol(FIPA, 1996; Smith, 1980)은 레퍼런스 작업 시장(task-market) 메커니즘이다. 서술 슬롯에 LLM을 꽂으면 현대적 LLM 기반 작업 시장이 된다.
...
이는 고전적 게임 이론(Aumann 1976, 합리성과 정보)의 2026 메아리다. 자신의 사적 평가(private valuation)를 드러내면 보수가 든다. LLM은 이를 직관하지 못하고, 상대에게 보이게 되는 추론 추적에 자신의 유보값을 즐겁게 타이핑한다.
...
| BATNA | "협상된 합의에 대한 최선의 대안" | 이 거래가 실패할 때의 폴백(fallback). 당신의 유보 가격을 정한다. |
