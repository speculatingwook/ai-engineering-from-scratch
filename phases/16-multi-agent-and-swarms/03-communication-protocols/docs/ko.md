...
당신은 시스템을 여러 에이전트로 쪼갰다. 연구자, 코더, 리뷰어. 그들은 각자의 일에는 뛰어나다. 하지만 이제 그들이 실제로 서로 대화하도록 해야 한다.

당신의 첫 시도는 뻔하다: 문자열을 주고받는다. 연구자가 텍스트 덩어리를 반환하면, 코더가 어떻게든 파싱한다. 코더가 연구 요약을 잘못 해석하거나, 두 에이전트가 서로를 기다리며 교착(deadlock)하거나, 서로 다른 팀이 만든 에이전트들이 협업해야 하기 전까지는 동작한다. 갑자기 "그냥 문자열을 주고받기"가 무너진다.

이것이 통신 프로토콜(communication protocol) 문제다. 에이전트가 정보를 교환하는 방식에 대한 공유 계약이 없으면, 멀티 에이전트(multi-agent) 시스템은 취약하고, 감사할 수 없으며, 당신이 직접 작성한 소수의 에이전트를 넘어 확장하는 것이 불가능하다.
...
A2A는 **피어 투 피어(peer-to-peer) 에이전트 협업**을 위한 프로토콜이다. MCP가 에이전트를 도구에 연결한다면, A2A는 에이전트를 다른 에이전트에 연결한다. 각 에이전트는 잘 알려진 URL에 **에이전트 카드(Agent Card)**를 게시하고, 다른 에이전트들이 그것을 발견하고, 협상하고, 작업을 위임한다.
...
주목할 핵심:
- **Skills**는 에이전트가 할 수 있는 일이다. 각각은 ID, 태그, 지원하는 입력/출력 MIME 타입을 갖는다. 이것이 클라이언트 에이전트가 이 원격 에이전트가 자신의 요청을 처리할 수 있는지 판단하는 방법이다.
- **supportedInterfaces**는 여러 프로토콜 바인딩을 나열한다. 단일 에이전트가 JSON-RPC, REST, gRPC를 동시에 말할 수 있다.
- **Security**는 카드에 내장되어 있다. 클라이언트는 단 한 번의 요청을 보내기 전에 어떤 인증이 필요한지 안다.
...
규제 산업에서 이것은 금광이다. 모든 답변은 증명 가능한 추론 사슬과 함께 온다: 어떤 도구가 호출되었고, 어떤 입력이 쓰였고, 어떤 출력이 받아졌는지. 블랙박스가 없다.
...
ANP는 **탈중앙 신원 프로토콜**이다. W3C 탈중앙 식별자(Decentralized Identifiers, DID)와 종단 간 암호화(end-to-end encryption)를 사용해 신뢰를 구축한다. 알려진 엔드포인트를 통해 에이전트를 발견하는 A2A와 달리, ANP는 에이전트가 자신의 신원을 암호학적으로 증명하게 한다.
...
- **`humanAuthorization`**은 ANP에 고유하다. 이 키들은 사용 전에 명시적 인간 승인(생체 인증, 비밀번호, HSM)을 요구한다. 자금 이체 같은 고위험 작업이 이 경로를 거친다.
...
이것이 ANP의 가장 새로운 기능이다. 서로 다른 생태계의 두 에이전트가 만날 때, 그들은 사전에 합의된 데이터 포맷이 필요하지 않다. 자연어로 협상한다.
...
에이전트들은 포맷에 합의할 때까지 오가며(최대 10라운드) 협상하고, 그다음 이를 처리할 코드를 동적으로 생성한다. 상태 값: `negotiating`, `rejected`, `accepted`, `timeout`.

이는 서로를 한 번도 본 적 없는 두 에이전트가, 누구도 공유 스키마를 사전 정의하지 않고도 어떻게 통신할지 알아낼 수 있음을 뜻한다.
...
- **ANP**는 당신이 통제하지 않는 에이전트에 대한 신원 검증을 제공한다
...
**스키마 표류(schema drift).** 에이전트 A가 `application/json` 출력을 광고하는 에이전트 카드를 게시한다. 그러나 JSON 스키마가 버전 간에 바뀐다. 에이전트 B는 옛 포맷을 파싱하여 쓰레기를 얻는다. 해결: 스킬과 출력 스키마에 버전을 매겨라. A2A 명세가 에이전트 카드에 `version`을 지원하는 것은 이 이유 때문이다.

**상태 기계 위반.** 에이전트 핸들러가 `completed` 이벤트를 산출한 뒤 더 많은 아티팩트를 산출하려 한다. 작업은 불변이다. 당신의 코드는 조용히 갱신을 버리거나 예외를 던진다. 해결: 산출 전에 종단 상태를 확인하라. 위의 `TaskManager`는 종단 상태 뒤의 `break`로 이를 강제한다.
...
**궤적 비대화(trajectory bloat).** ACP 궤적 로깅은 강력하지만 비싸다. 실행당 200번의 도구 호출을 하는 복잡한 에이전트는 거대한 감사 항목을 생성한다. 해결: 설정 가능한 상세도 수준에서 궤적을 로깅하라. 컴플라이언스를 위해 도구 이름과 IO를 기록하되, 비규제 워크로드에서는 추론 단계를 건너뛰어라.

**발견 천둥 떼(thundering herd).** 50개 에이전트가 시작 시 동시에 `GET /agents`를 질의한다. 해결: TTL과 함께 에이전트 카드를 캐시하고, 발견 간격을 시차를 두거나, 폴링 대신 푸시 기반 등록을 사용하라.

## 라이브러리로 써보기 (Use It)

### 실제 구현들

**A2A**가 가장 성숙하다. Google의 [공식 명세](https://github.com/google/A2A)는 Linux Foundation 산하 오픈소스다. Python과 TypeScript SDK가 있다. 에이전트가 동적 발견과 협업이 필요하다면 여기서 시작하라.
...
- `outputs/prompt-protocol-selector.md` -- 당신의 시스템에 맞는 프로토콜을 고르도록 돕는 프롬프트
