...
프로토타입 다중 에이전트 시스템은 인메모리 이벤트 루프에서 세 에이전트로 노트북 한 대에서 작동한다. 당신은 프로덕션(production)으로 옮긴다.
...
LLM 호출은 I/O 제약적이다. 다음 토큰을 기다리는 스레드는 99%의 시간 동안 유휴 상태다. 스레드는 각각 약 1MB RAM이 든다. 10,000개의 동시 호출에서, 그것은 스택만으로 10GB다.

파이버(Python `asyncio`, Go 고루틴, Rust `tokio`)는 I/O에서 협력적으로 양보한다. 같은 10,000개 호출이 프로세스 안에 편안하게 들어간다. LLM 에이전트 규모에서, 비동기는 최적화가 아니다 — 그것이 아키텍처다.
...
감당 가능한 과제에서 약 100개 미만의 동시 에이전트 실행 부하의 경우, 이것이 종종 당신에게 필요한 전부다. 그것이 실패하는 것을 측정하면 업그레이드하라.

규칙: 단순한 아키텍처가 풀 수 없는 구체적 문제에 부딪혔을 때 내구성 실행 프레임워크를 채택하라. 조기 채택은 성과를 내지 못하는 의식에 시간을 태운다.
...
이것들은 LLM 특화가 아니라 데이터베이스 엔지니어링 패턴이다. LLM 세금은 LLM 호출이 느리다는 것뿐이다. 나머지는 모두 표준 분산 시스템이다.
...
- **단순하게 시작하라(Bedi의 규칙).** 그것이 실패하는 것을 측정하기 전까지는 FastAPI + Postgres.
...
4. LangGraph의 런타임 문서(아래 링크)를 읽어라. 런타임의 어떤 기능이 직접 만든 FastAPI + Postgres 버전에서 복제하는 데 가장 오래 걸릴지 식별하라. 그것이 채택할 이유인가, 아니면 미룰 수 있는가?
