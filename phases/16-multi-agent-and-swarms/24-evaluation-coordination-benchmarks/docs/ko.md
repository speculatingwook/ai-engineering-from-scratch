...
에이전트들이 서로 다른 관찰 모달리티를 가지고 완전한 정보 공유 없이 협응해야 하는 과제를 다룬다. 보고된 결과는 불편하다: COMMA의 에이전트-에이전트 협업에서 GPT-4o를 포함한 프런티어 모델들이 **무작위 베이스라인**을 이기는 데 고전한다. 신호는 다중 에이전트 모달리티가 학습이 부족하고 평가가 부족하다는 것이다 — LLM은 단일 모달리티 협력은 그럭저럭 다루지만, 다중 모달리티 협응은 무너진다.

사용하는 경우: 당신의 시스템에 멀티모달 또는 비대칭 정보 협응이 있을 때. COMMA의 영(null) 결과는 주장하기 전에 측정하라는 경고다.
...
사용하는 경우: 당신의 도메인에 명확한 단일 LLM 베이스라인이 있을 때. MedAgentBoard의 교훈이 일반화된다면, 많은 제안된 다중 에이전트 시스템이 과잉 설계된 것이다.
...
사용하는 경우: 엔터프라이즈 에이전트 스택을 설계 중이고 각 계층을 정당화해야 할 때. AgentArch는 가치를 측정할 수 없는 기능을 구매하는 것을 피하도록 돕는다.
...
당신이 실제로 신경 쓰는 축에 대해 자체 내부 벤치마크를 만드는 것이 종종 옳은 수다.
...
- **내부 벤치마크를 만들어라** — 당신의 실제 프로덕션 분포를 반영하는. 공개 벤치마크는 정보를 주지만 대체하지 못한다.
...
- **공개 벤치마크 과적합(overfitting)을 피하라.** 당신의 팀이 SWE-bench Pro 숫자를 위해 특별히 최적화하고 있다면, 프로덕션에서 퇴보할 것이다.

## 연습 문제 (Exercises)

1. `code/main.py`를 실행하라. 세 시뮬레이션 시스템 중 마일스톤당 비용이 가장 좋은 것을 식별하라. 그것이 가장 높은 원시 정확도 시스템과 일치하는가?
2. MultiAgentBench (arXiv:2503.01935)을 읽어라. 당신 자신의 과제 도메인에 대해, MARBLE이 네 토폴로지 중 어느 것을 권할지 결정하라. 논문의 결과로부터 정당화하라.
3. SWE-bench Pro 논문을 읽어라. 무엇이 구체적으로 그것을 오염 저항적으로 만드는가? 같은 기법을 당신이 신경 쓰는 다른 벤치마크에 적용할 수 있는가?
