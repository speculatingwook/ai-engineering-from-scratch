...
Phase 14에서 단일 에이전트(single agent)를 만들었다. 잘 동작한다. 파일을 읽고, 명령을 실행하고, API를 호출하고, 결과를 놓고 추론할 수 있다. 그런 다음 실제 코드베이스에 이 에이전트를 들이댄다. 파일 200개, 세 가지 언어, 인프라에 의존하는 테스트들, 그리고 코드를 작성하기 전에 외부 API를 조사해야 한다는 요구사항.
