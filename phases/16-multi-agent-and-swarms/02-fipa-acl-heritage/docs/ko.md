...
솔직한 해석은, 그들 대부분이 매우 구체적인 20년 된 결정 트리를 재발견하고 있다는 것이다. 오스틴(Austin, 1962)과 설(Searle, 1969)의 발화 행위 이론(speech-act theory)은 우리에게 "발화는 행위다"를 주었다. KQML(1993)은 이를 와이어 프로토콜(wire protocol)로 바꿨다. FIPA-ACL(2000년 비준)은 참조 표준화를 만들어냈다: 스무 개의 수행문, 콘텐츠 언어 SL0/SL1, contract-net과 subscribe-notify를 위한 상호작용 프로토콜. JADE와 JACK은 자바 참조 플랫폼이었다. 온톨로지 오버헤드가 너무 무거웠고 웹이 이기고 있었기에, 이 노력은 2010년경 사라졌다.

MCP의 `tools/call`, A2A의 작업 생명주기, 혹은 CA-MCP의 공유 컨텍스트 저장소를 볼 때, 당신은 FIPA 결정들의 더 부드럽고 JSON 네이티브한 재탕을 보고 있는 것이다. 이 유산을 아는 것은 두 가지를 알려준다: 어떤 새로운 "혁신"이 사실은 재발명인지, 그리고 새 명세들이 어떤 오래된 실패 양식을 재발견하게 될지.
...
전체 목록은 `fipa00037.pdf`(FIPA ACL Message Structure)에 있다. 요점은 이를 외우는 것이 아니다 — 요점은 이들 각각이 결국 LLM 프로토콜이 다시 추가하게 되는 프리미티브(primitive)에 대응한다는 것이다.
...
일곱 개의 필드가 프로토콜 봉투(envelope)를 운반하고, 한 필드(`content`)가 페이로드(payload)를 운반한다. 나머지 필드들은 당신이 JSON 프로토콜에 재시도, 스레딩, 온톨로지를 덧붙일 때마다 재발명하는 바로 그것이다.
...
같은 봉투, 다른 문법. 둘 다 다음을 운반한다: 누가, 누구에게, 의도, 페이로드, 상관 id(correlation id). 어느 쪽도 다른 쪽에 대한 혁명이 아니다 — 그들은 같은 설계에 대한 다른 트레이드오프(trade-off)다.
...
더 쉬운 구현을 위한 더 느슨한 의도 의미론. 그것이 바로 그 거래다.
...
`outputs/skill-fipa-mapper.md`는 임의의 에이전트 프로토콜 명세를 읽어 FIPA-ACL 매핑을 만들어내는 스킬이다. 새 프로토콜을 채택하기 전에 이를 사용해 "이것이 진정 새로운가, 아니면 JSON 문법을 입은 `inform`인가?"에 답하라.
...
5. 당신만의 시스템에서 `request` 수행문의 `content` 필드를 위한 최소 JSON-Schema를 설계하라. 그 스키마가 순수 자연어가 주지 않는 무엇을 주며, 무엇을 치르게 하는가?
