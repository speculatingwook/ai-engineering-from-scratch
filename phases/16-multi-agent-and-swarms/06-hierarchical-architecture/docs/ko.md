...
문제: LLM 매니저는 인간 매니저와 같지 않다. 인간 매니저는 자신의 부하 직원이 무엇을 아는지에 대한 안정적인 사전 지식(prior)을 가진다. LLM 매니저는 매 턴마다 자신의 컨텍스트에 있는 무엇이든으로부터 조직을 다시 추론한다. 그 컨텍스트의 작은 표류(drift)만으로, 트리 전체가 작업을 잘못 배분한다.
...
1. **작업 할당 오류.** 매니저가 목표를 읽고, 분해를 환각(hallucinate)하고, 잘못된 서브 매니저에게 위임한다. 서브 매니저가 받은 것에 순순히 작업하기 때문에, 오류는 오직 최상위 종합에서야 드러난다 — 인간이 잡을 수 있었던 곳에서 한 단계 떨어진 지점이다.
...
순차형(선형 파이프라인) 대 계층형: 당신의 작업에 실제로 독립적인 하위 팀이 있는가, 아니면 트리인 척하는 하나의 선형 흐름인가? 후자라면, 순차형을 사용하라. 전자라면, 계층형을 쓰되 명시적 조정 규칙에 예산을 두어라.
...
데모는 정상 경로(모두가 동의)와, 최상위 매니저의 분해가 "legal"을 "finance"로 잘못 레이블링하는 **교란된 경로(perturbed path)**를 대조하고 오류가 연쇄되는 것을 지켜본다 — 서브 매니저가 순순히 재무 작업을 하고, 최상위 종합자가 재무 발견을 보고하고, 원래의 법무 질문은 답해지지 않는다.
...
- **트리 깊이를 2로 상한.** 세 단계만 되어도 이미 대부분의 오류가 관측 가능성(observability)에서 숨는다.
