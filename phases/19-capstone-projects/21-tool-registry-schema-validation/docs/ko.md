...
우리가 피하는 실수는 스키마 없이 핸들러를 출하하거나, 검증 없이 스키마를 출하하는 것이다. 둘 다 흔하다. 둘 다 다음 계층(23번 레슨의 디스패처)을 유일한 실패 모드가 핸들러로부터의 스택 트레이스(stack trace)인 추측 게임으로 만든다.
...
스키마는 검증기가 건드리는 유일한 필드다. 핸들러는 검증기에게 불투명(opaque)하다. 우리는 그것들을 의도적으로 분리한다. 스키마는 데이터다. 핸들러는 코드다. 둘을 섞으면 검증 로직을 핸들러 안에 넣고 싶은 유혹이 생기는데, 그것이 바로 우리가 막는 버그다.
...
그것으로 도구 API가 실제로 필요로 하는 것을 다루기에 충분하다. 우리가 추가하지 않는 키워드(oneOf, anyOf, allOf, $ref, 조건문)는 프로덕션 스키마에서 유효하지만 검증기를 순환(cycle)이 있는 트리 워커(tree walker)로 만든다. 우리는 레지스트리를 만드는 것이지 JSON Schema 엔진을 만드는 것이 아니다.

## Json pointer 오류 경로 (Json pointer error paths)

검증이 실패하면, 검증기는 오류 목록을 반환한다. 각 오류는 입력으로의 json-pointer 경로를 지닌다. 포인터는 슬래시 접두(slash-prefixed)의 프로퍼티 이름과 배열 인덱스의 순서다.
...
그것은 스키마 트리에 대한 단일 패스이며, 재귀적이다. 그것은 순수하다. 핸들러를 호출하지 않는다. 타입을 강제 변환(coerce)하지 않는다(문자열 `"42"`는 number 스키마를 통과하지 못한다). 조용히 잘라내지(truncate) 않는다.

그것은 보안 경계(security boundary)가 아니다. 악의적 핸들러는 검증이 통과한 후에도 여전히 오작동할 수 있다. 23번 레슨의 디스패처가 타임아웃(timeout)과 샌드박스(sandbox) 계층을 추가한다. 레지스트리는 형태를 추가한다.
...
이 레슨이 안착하면 당신이 원하게 될 두 확장은 로컬 정의 블록(definitions block)에 대한 `$ref` 해소(resolution)와 엄격한 형태를 위한 `additionalProperties: false`다. 둘 다 작다. 둘 다 도구 카탈로그가 50개 도구를 넘어 성장할 때 추가하기에 흔하다. 우리는 파일을 한 번의 읽기 분량 아래로 유지하기 위해 레슨에서 그것들을 뺐다.
