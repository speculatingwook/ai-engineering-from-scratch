...
당신은 작은 트랜스포머(transformer)를 일반적인 말뭉치(corpus)에서 사전 학습했다. 출력 헤드는 마지막 은닉 상태(hidden state)를 1000-토큰 어휘(vocabulary)로 투영(project)한다. 이제 스팸 또는 햄으로 레이블링된 SMS 메시지 800개가 있고 이진(binary) 분류기를 원한다. 세 가지 선택지가 존재한다.

잘못된 선택지는 800개 예제에서 밑바닥부터 새 분류기를 학습시키는 것이다. 사전 학습된 모델의 몸통은 이미 유용한 구조를 인코딩한다. 단어 정체성, 위치, 단순 동시 출현(co-occurrence). 그것을 버리는 것은 그것을 만든 연산을 낭비한다.
...
몸통은 의도적으로 작은 트랜스포머다. 어휘 260, 은닉 64, 4 헤드, 2 블록, 최대 시퀀스 32. CPU에서 90초 안에 두 체제를 모두 수렴(convergence)까지 학습시킬 만큼 작다. 레슨에서 사전 학습되지는 않는다. 대신, `pretrain_quick` 도우미(helper)가 같은 픽스처의 텍스트에서 LM 학습을 다섯 에폭(epoch) 수행하여 몸통에 비자명한(non-trivial) 출발점을 준다. 이것이 레슨을 독립적(self-contained)으로 유지한다.
...
구현은 당신에게 훅(hook)을 준다. 테스트는 계약을 고정(pin)한다. 수치는 당신이 밀어붙일 몫이다.
