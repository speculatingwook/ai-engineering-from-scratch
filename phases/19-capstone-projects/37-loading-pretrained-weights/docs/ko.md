...
게시된 가중치는 당신의 아키텍처에 맞게 포장되어 있지 않다. 그것들은 원래 구현이 쓴 이름을 지닌다. 사전 학습된 파일은 형태 `(2304, 768)`의 `transformer.h.0.attn.c_attn.weight`를 갖는다. 당신의 모델은 형태 `(2304, 768)`의 `blocks.0.attn.qkv.weight`(다른 레이아웃 관습의 같은 행렬)를 기대하거나, 당신의 모델은 행렬을 전치(transpose)하여 저장하는 `nn.Linear`를 쓴다. 같은 파라미터가 미묘하게 다른 세 가지 정체성(이름, 형태, 바이트 레이아웃)으로 나타나며 로더는 셋 모두를 조정해야 한다.

맹목적으로 복사하는 로더는 올바른 텐서를 잘못된 곳에 넣어 헛소리를 생성하는 모델을 만든다. 형태가 다를 때 복사를 거부하지만 아무것도 로깅하지 않는 로더는 어떤 텐서가 안착에 실패했는지 추측하게 한다. 이 레슨의 로더는 명시적이다. 모든 할당이 로깅되고, 모든 형태가 검사되며, `LoadReport`가 히트(hit), 미스(miss), 형태 불일치를 요약하여 무슨 일이 일어났는지 읽을 수 있게 한다.
...
세 가지 패턴이 당신이 만들지 않은 가중치와의 접촉에서 로더를 살아남게 한다.
...
- 로드 후 정상성 생성은 빠른 게이트(gate)다. 로드 후 샘플이 로드 전 샘플처럼 보이면, 로드가 모델을 바꾸지 않은 것이며, 이는 매핑이 모든 텐서를 조용히 놓쳤다는 뜻이다.
...
5. `NAME_MAP`을 확장하여 LLaMA 이름 관습(편향 없음, RMSNorm, 융합 qkv 레이아웃)을 처리하고, 당신이 생성한 스텁 LLaMA 픽스처에서 로더를 다시 실행하라.
