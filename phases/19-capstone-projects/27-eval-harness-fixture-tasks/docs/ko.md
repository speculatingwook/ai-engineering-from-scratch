...
첫 번째는 검증되지 않은 통과다. 에이전트가 버그를 고쳤다고 말하고, 사람이 diff를 흘끗 보고, 작업 모음이 초록색으로 표시되며, 3주 뒤 회귀 테스트가 같은 버그를 드러낸다. 에이전트는 실제로 무언가를 고치지 않은 채 그럴듯하게 추론했던 것이다.
...
하니스는 각 작업을 `k`번 실행한다. Pass@k는 `1 - (1 - p)^k`이며 여기서 p는 경험적(empirical) 통과율이다. 하니스는 분산(variance)을 파악할 수 있도록 원시 횟수도 함께 보고한다. 지연 시간은 샘플당 벽시계 시간(wall-clock)이다. 비용은 에이전트가 스스로 보고하는 무엇이든이다(토큰 수, USD, 또는 둘 다). 하니스는 그것을 샘플 전체에 걸쳐 합산하고 작업별 및 집계 수치를 제시한다.
...
실제 LLM 에이전트는 확률적(stochastic)이다. pass@1이 0.6이면 실패처럼 보인다. pass@5가 0.95라면 에이전트가 대부분의 경우 올바른 답을 내지만 초기 샘플에서 잘못 선택하고 있다는 뜻이다. 해법은 항상 더 많은 학습이 아니라 샘플링(sampling)과 순위 매기기(ranking)다. Pass@k는 그것을 가시화한다.

Pass@k는 pass@1과 나란히 보고되는데, pass@k가 실제 실패를 덮어 가리기 때문이다. 모델이 스무 번 시도해서 한 번 올바른 답을 낸다면 쓸 만한 에이전트를 가진 것이 아니다. 하니스는 둘 다 보여 준다.
