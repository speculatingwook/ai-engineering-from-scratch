...
마흔 턴 동안 무인으로 실행되는 코딩 에이전트는 채팅 루프가 아니다. 그것은 운영자가 노드를 가로챌 수 있고 운영자가 간선을 감사할 수 있는 상태 기계다. 계약을 적어두는 순간, 모델·도구·정책을 교체하는 일은 더 이상 리팩터(refactor)가 아니게 된다. 그것은 등록(registration) 호출이 된다.
...
상태 기계는 결정론적이다. 동일한 이벤트 로그가 주어지면, 하네스는 동일한 상태로 다시 진입한다. 그 속성이 모델을 다시 호출하지 않고 디버깅을 위해 세션을 재생(replay)할 수 있게 하는 것이다.
...
풀 포인트는 예외가 아니다. 그것은 반환(return)이다. 호출자는 하네스 상태를 검사하고, 하네스가 요청한 것을 가져온 뒤, `resume(payload)`를 호출한다. 하네스는 멈춘 곳에서 다시 시작한다. 이것은 Python 제너레이터(generator)와 동일한 형태다. 풀 포인트 너머의 전송은 당신의 선택이다. TUI에서는 키 입력이다. MCP에서는 `tools/call`이다. 큐에서는 작업 폴(job poll)이다.
...
예산은 킬 스위치(kill switch)가 아니다. 그것은 양보(yield)다. 호출자가 예산을 연장하고 재개할지, 아니면 세션을 닫을지 결정한다.
...
프로덕션에서 하네스를 만들 때 가장 어려운 부분은 상태 기계가 아니다. 계약을 시행 가능하게 만드는 것이다. 계약은 플래너의 핫 리로드(hot reload)에서 살아남아야 한다. 잘못된 형식의 JSON을 반환하는 도구에서 살아남아야 한다. 마흔 턴 세션의 3분의 2 지점에서 `before_tool_call`에서 예외를 일으키는 훅에서 살아남아야 한다. 이 레슨의 테스트는 그 실패 모드들을 동작시킨다. 실행하라. 깨뜨려라. 케이스를 추가하라.
