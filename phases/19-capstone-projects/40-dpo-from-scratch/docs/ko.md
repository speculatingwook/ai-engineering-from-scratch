...
당신에게 SFT 모델이 있다. 그것은 인스트럭션(instruction)을 따르지만, 출력이 들쭉날쭉하다. 어떤 완성물은 명료하고, 어떤 것은 장황하거나 틀렸다. 또한 선호 쌍의 작은 데이터셋(dataset)이 있다. 같은 프롬프트에 대해, 사람이 하나의 완성물을 선택됨으로, 다른 하나를 거부됨으로 표시했다.

고전적인 RLHF 답은 2단계 파이프라인(pipeline)이다. 선호에서 보상 모델을 학습시킨다. PPO로 보상에 대해 정책을 최적화한다. 이것은 동작하지만 비싸다. PPO 동안 메모리에 두 모델, 정책을 참조 가까이 유지하기 위한 KL 제어, 보상 모델이 취약할 때의 보상 해킹(reward hacking).
...
DPO 유도는 이 목적 아래의 최적 정책 `pi*`가 `r`의 항으로 닫힌 형태(closed form)를 가짐을 관찰한다.
...
이것이 손실이다. 그것은 예제당 단일 스칼라(scalar)에 대한 시그모이드이며, 네 개의 로그 확률에서 계산된다. 별개의 보상 모델 없음. PPO 없음. 손실에 KL 항 없음. KL 제약은 닫힌 형태 유도에 구워 넣어진다.
...
여기서 `z`는 시그모이드의 인자다. 이것은 모든 `z`에 대해 음수이며, 이는 곧 선택된 완성물에 대한 정책의 로그 확률을 높이면 손실이 감소함을 뜻한다. 대칭적으로, `log pi_theta(y_l | x)`에 대한 그래디언트는 양수다. 거부된 로그 확률을 높이면 손실이 증가한다. 학습은 선택된 것을 위로, 거부된 것을 아래로 민다. 참조는 동결되어 있다. 움직이지 않는다.
...
픽스처는 의도적으로 작다. DPO는 프로덕션(production)에서 수만 개 쌍에 대해 동작한다. 여기서의 요점은 손실 수학과 루프(loop)가 작은 데이터셋에서 종단 간(end-to-end)으로 실행되고 선택된 대 거부된 로그 확률 간격이 눈에 띄게 커지는 것이다.

## 참조 불변성 (Reference Invariance)

DPO 구현은 참조 모델을 신중하게 다뤄야 한다. 참조는 제자리에 동결된 SFT 모델이다. 세 가지 속성이 유지되어야 한다.
...
모델은 레슨 39에서 쓴 같은 TinyGPT(디코더 전용(decoder-only), 인과(causal), 바이트 토크나이저(byte tokeniser))다. 참조와 정책은 아키텍처를 공유한다. 정책의 가중치는 학습 아래에서 참조로부터 표류(drift)하지만 참조는 고정된 채 머문다.
...
DPO는 보상의 매개변수화(parameterisation)까지, Bradley-Terry 선호 모델 아래에서 RLHF와 수학적으로 동등하다. 암묵적 보상 `r(x, y) = beta * (log pi(y|x) - log pi_ref(y|x))`는 차이에서 상쇄되는 `x`의 함수까지, 선호로부터 식별 가능(identifiable)하다. 닫힌 형태 정책은 명시적 보상 모델을 건너뛰게 한다. KL 제약은 구조적으로 강제된다. `pi`가 `pi_ref`에서 벗어나면 로그 비율이 커지고, 시그모이드가 포화(saturate)되어 정책이 너무 멀리 움직일 때 그래디언트를 감쇠시킨다. 참조가 당신의 안전망이다.

## 스트레치 목표 (Stretch goals)

- 로그 확률 합에 길이 정규화(length normalisation)를 추가하라: 완성물 길이로 나눈다. 길이 편향(length bias)은 모델이 더 짧은 완성물을 선호적으로 선택하는 알려진 DPO 실패 모드인데, 그것들의 로그 확률이 절대값에서 더 크기 때문이다.
...
구현은 당신에게 손실, 참조 불변성, 학습 루프를 준다. 수학이 레슨이다. 코드는 수학을 구체적으로 만든다.
