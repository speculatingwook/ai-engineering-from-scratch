...
당신이 실제로 원하는 파이프라인은 이 네 가지를 모두 갖춘다. 각 평가는 다른 평가가 놓치는 차원을 다룬다. 각각은 그 지표에 맞게 구성된 홀드아웃 데이터의 서로 다른 부분집합에서 수행된다. 최종 리포트는 태스크별 수치를 나란히 보여주고 집계값도 보여주므로, 리뷰어는 모델이 어떤 트레이드오프(trade-off)를 하고 있는지 한눈에 알 수 있다.
