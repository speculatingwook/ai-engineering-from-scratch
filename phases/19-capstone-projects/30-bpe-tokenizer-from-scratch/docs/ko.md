...
우리는 바이트 수준 변형을 만들 것이다. 알파벳은 유니코드 코드 포인트(code point)가 아니라 256개의 원시 바이트다. 그 선택이 토크나이저가 알 수 없는(unknown) 토큰으로 후퇴하지 않고 어떤 UTF-8 입력이든 다룰 수 있게 해 준다.
...
사전 토크나이저(pretokenizer)는 학습이 보기 전에 공백과 구두점 경계에서 말뭉치를 분할한다. 그 분할이 없으면 BPE 병합 단계는 단어 경계를 넘나드는 병합을 기꺼이 학습하고 어휘가 통째로 된 흔한 구절들로 가득 찬다. 분할이 있으면 병합이 단어 안에 머무르고 결과가 일반화(generalize)된다.
...
쌍 카운터를 병렬화하지 않는다. 수천 단어의 말뭉치에 대한 파이썬 루프는 1초 안에 훨씬 못 미쳐 끝난다. 더 큰 말뭉치의 경우 명백한 수는 단어별로 쌍을 병렬로 세고 리듀스(reduce)하는 것이다.
