...
모델은 한 디바이스에 들어간다. 데이터셋은 그렇지 않다. 최적화 예산은 월클록(wallclock) 초당 N배의 예제를 보고 싶다고 말한다. 첫 번째 지렛대는 데이터 병렬(data parallel)이다. 각 랭크는 배치의 다른 조각에 대해 같은 모델을 돌린 뒤, 옵티마이저 스텝 전에 그래디언트를 평균한다. 두 번째 지렛대는 FSDP다. 모델도 한 디바이스에 들어가지 않으므로, 각 랭크가 모든 파라미터의 일부를 보유하고 순방향 패스 동안 층(layer)별로 전체 텐서를 재구성한다.
...
이 레슨은 CPU에서 돌아간다. CUDA는 가정되지 않는다. `gloo` 백엔드는 모든 PyTorch 빌드와 함께 출시되고 `torch.multiprocessing` 워커를 받아들인다. 같은 코드가 다중 GPU 노드에서 구조 변경 없이 `nccl`로 전환된다.
...
N개 랭크에 걸쳐 B개 예제 배치로 학습된 모델은 N*B 배치로 학습하는 단일 프로세스와 같은 그래디언트를 만들어야 한다. 요령은 랭크별 그래디언트를 합하고 N으로 나누면 평균 손실 그래디언트를 주는데, 이것이 평균 리덕션(mean reduction)을 가진 교차 엔트로피(cross entropy)가 전체 배치에서 만들었을 것이라는 점이다. 레슨 코드는 수동 all-reduce 그래디언트와 레퍼런스 단일 프로세스 그래디언트 사이를 `max-abs-diff < 1e-3`으로 단언한다.
...
메모리 이득은 정확하다. 파라미터에 대한 랭크별 메모리가 1/N로 떨어진다. 비용은 매 순방향 패스마다 치르는 모으기(gather)다. 프로덕션 FSDP는 이전 층의 계산과 모으기를 겹쳐서 월클록 비용이 순진한 계산이 예측하는 것보다 훨씬 작다. 레슨은 모든 파라미터에 대해 왕복을 수행하고 재구성이 원본과 비트 단위로 같음을 단언한다.
