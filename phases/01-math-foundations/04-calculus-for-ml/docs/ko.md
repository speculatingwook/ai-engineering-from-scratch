...
도함수는 변화율(rate of change)을 측정한다. 함수 y = f(x)에 대해, 도함수 f'(x)는 알려준다: x를 아주 작은 양만큼 밀면 y는 얼마나 변하는가?
...
실제 함수는 입력이 많다. 신경망의 손실은 수천 개의 가중치에 의존한다. 편도함수(partial derivative)는 하나를 제외한 모든 변수를 상수로 고정한 뒤, 그 하나에 대해 미분한다.
...
각 편도함수는 답한다: 이 하나의 가중치만 밀면 손실은 어떻게 변하는가?

### 그래디언트: 모든 편도함수의 벡터

그래디언트는 모든 편도함수를 하나의 벡터로 모은다. 함수 f(x, y, z)에 대해, 그래디언트는:
...
수치 도함수는 더 느리지만 어떤 함수에도 동작한다. 해석적 도함수는 빠르지만 공식을 직접 유도해야 한다. 신경망 프레임워크는 세 번째 접근법을 사용한다: 자동 미분(automatic differentiation)으로, 정확한 도함수를 기계적으로 계산한다. 이는 Phase 3에서 보게 된다.
...
뉴턴 방법은 더 빨리 수렴(convergence)한다. 헤시안이 그래디언트를 "재스케일링"하기 때문이다 — 가파른 방향은 더 작은 스텝을, 평평한 방향은 더 큰 스텝을 받는다.
...
연쇄 법칙은 한 줄로 늘어선 스칼라 함수에만 적용되는 게 아니다. 신경망에서 변수들은 갈라지고(fan out) 합쳐진다. 다음은 도함수가 간단한 순방향 패스(forward pass)를 통해 어떻게 흐르는지다:
...
신경망에 대해 야코비안을 손으로 계산하지는 않는다. PyTorch가 처리한다. 하지만 그것이 존재한다는 걸 알면 역전파에서의 형태(shape)를 이해하는 데 도움이 된다: 어떤 층이 R^n을 R^m으로 매핑하면, 그 야코비안은 m x n이다. 그래디언트는 이 행렬의 전치(transpose)를 통해 역방향으로 흐른다.

### 이것이 신경망에서 중요한 이유

신경망의 모든 가중치는 그래디언트를 받는다. 그래디언트는 손실을 줄이기 위해 그 가중치를 어떻게 조정할지 알려준다.
...
방금 당신은 경사 하강법을 밑바닥부터 만들었다. PyTorch는 그래디언트 계산을 자동화하지만, 갱신 루프는 동일하다.
