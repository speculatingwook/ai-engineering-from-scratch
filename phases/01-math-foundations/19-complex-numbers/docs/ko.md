...
복소수는 동시에 점이자 원점으로부터의 벡터다. 이 이중 해석이 복소수를 기하학에 유용하게 만든다.
...
이는 복소 지수가 곧 회전이라는 뜻이다. 그리고 회전은 신호 처리와 ML 어디에나 있다.
...
3. **알려진 신호의 DFT.** 32개 점에서 샘플링된 sin(2*pi*3*t)와 0.5*sin(2*pi*7*t)의 합인 신호를 만들어라. 당신의 DFT를 실행하라. 크기 스펙트럼이 주파수 3과 7에서 봉우리를 가지며, 7의 봉우리가 3의 봉우리 높이의 절반인지 검증하라.
