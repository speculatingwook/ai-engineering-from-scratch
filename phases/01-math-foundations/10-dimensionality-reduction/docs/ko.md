...
- 설명된 분산 비율(explained variance ratio)과 엘보(elbow) 방법을 사용해 주성분(principal component) 개수 선택하기
...
샘플당 784개 특성(feature)을 가진 데이터셋(dataset)이 있다. 손글씨 숫자의 픽셀 값일 수도 있다. 유전자 발현 수준일 수도 있다. 사용자 행동 신호일 수도 있다. 784차원을 시각화할 수 없다. 그릴 수 없다. 심지어 머릿속에 떠올릴 수도 없다.

하지만 그 784개 특성 대부분은 중복이다. 실제 정보는 훨씬 작은 표면 위에 산다. 손글씨 "7"을 묘사하는 데 784개의 독립적인 숫자가 필요하지 않다. 몇 개면 된다: 획의 각도, 가로 막대의 길이, 얼마나 기울어졌는지. 나머지는 노이즈다.
...
**지수적으로 더 많은 데이터가 필요하다.** 공간에서 같은 샘플 밀도를 유지하려면, 2D에서 20D로 가는 것은 10^18배 더 많은 데이터가 필요함을 뜻한다. 결코 충분하지 않다. 차원을 줄이면 데이터 밀도가 다룰 만한 수준으로 되돌아온다.
...
누적 설명된 분산이 0.95에 도달하면, 그만큼의 성분이 정보의 95%를 포착함을 안다. 그 이후의 모든 것은 대부분 노이즈다.
...
직관: 원래 공간에서 점 쌍들의 거리에 기반해 확률 분포를 계산한다. 가까운 점은 높은 확률을 받는다. 먼 점은 낮은 확률을 받는다. 그다음 같은 확률 분포가 성립하는 2D 배치를 찾는다. 784차원에서 이웃이었던 점들은 2D에서도 이웃으로 남는다.
...
- 더 나은 전역 구조. 출력에서 클러스터의 상대적 위치가 t-SNE보다 더 의미 있는 경향이 있다.

UMAP은 고차원 공간에서 가중 그래프("퍼지 위상 표현")를 만든 뒤, 이 그래프를 가능한 한 잘 보존하는 저차원 레이아웃을 찾는다.

핵심 파라미터:
- `n_neighbors`: 몇 개의 이웃이 국소 구조를 정의하는지(퍼플렉서티와 유사). 높은 값일수록 더 많은 전역 구조를 보존한다.
- `min_dist`: 출력에서 점들이 얼마나 빽빽하게 모이는지. 낮은 값일수록 더 밀집된 클러스터를 만든다.
...
고전적인 예시: 2D의 동심원. 두 개의 점 고리, 하나가 다른 하나 안에 있다. 표준 PCA는 둘 다 같은 직선에 사영한다 — 분류에는 쓸모없다. RBF 커널을 사용한 커널 PCA는 안쪽 원과 바깥쪽 원을 서로 다른 영역으로 매핑하여, 선형 분리 가능하게 만든다.

### 재구성 오차 (Reconstruction Error)

당신의 차원 축소는 얼마나 좋은가? 784차원을 50차원으로 압축했다. 무엇을 잃었는가?
...
재구성 오차는 k를 고르는 것 이상으로 유용하다. 이상 탐지(anomaly detection)에 쓸 수 있다: 재구성 오차가 높은 샘플은 학습된 부분공간에 맞지 않는 이상치(outlier)다. 이것이 프로덕션(production) 시스템에서 PCA 기반 이상 탐지의 토대다.
...
성능은 784차원에 한참 못 미쳐서 평탄해진다. 그 평탄해지는 지점이 당신의 작동점이다.
