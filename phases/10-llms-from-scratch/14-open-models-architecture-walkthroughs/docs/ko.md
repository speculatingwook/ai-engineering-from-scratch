...
> 당신은 Lesson 04에서 GPT-2 Small을 밑바닥부터 만들었다. 2026년의 프런티어 오픈 모델은 다섯 또는 여섯 개의 구체적 변경을 가진 같은 가족이다. LayerNorm 대신 RMSNorm. GELU 대신 SwiGLU. 학습된 위치 대신 RoPE. 완전한 MHA 대신 GQA나 MLA. 규모에서의 전문가 혼합(Mixture-of-Experts). 당신이 이미 아는 수학이 그중 95%를 다룬다. 이 레슨은 Llama 3, DeepSeek-V3, Mixtral, Qwen, Gemma를 나란히 읽고 각 아키텍처가 갈라지는 정확한 줄을 짚는다.
...
Lesson 04에서 당신은 350줄의 numpy를 작성해 GPT-2 모양의 모델을 가졌다. Llama 3 405B는 200페이지짜리 기술 보고서를 가진다. 당신의 직감은 이것들이 다른 짐승이라는 것이다. 아니다. 그 200페이지는 다섯 또는 여섯 개의 충분히 동기 부여된 수정과, 확장에 관한 천 개의 구현 세부사항을 더한 같은 객체를 기술한다. 골격 — 임베딩(embedding), 트랜스포머 블록, 어텐션(attention), MLP, 정규화(norm), 헤드(head) — 은 변하지 않는다.

이 레슨은 하나의 diff다. 각 주요 오픈 모델 가족에 대해, GPT-2에서 무엇이 바뀌었는지, 왜, 무엇을 대가로 치렀는지 정확히 나열한다. 끝나면 새로운 모델 카드를 읽고 그것을 GPT-2 베이스라인(baseline)으로 머릿속에서 되번역할 수 있다.

실용적 보상은 Meta가 Llama 5를 출시하거나 DeepSeek이 V4를 출시할 때, 새로운 사고 모델이 필요하지 않다는 것이다. 설정을 보고, 잘 알려진 손잡이(knob) 중 어느 것이 움직였는지 보고, 하류 함의가 무엇인지 안다. 2026년 아키텍처는 유한한 도구 상자다. 각 새 모델은 다른 부분집합을 고른다.
...
약 13B 파라미터 이상의 모든 모델에서, GQA나 MLA는 사실상 필수다. 규모에서의 완전한 MHA는 KV 캐시 재앙이다.
...
매력: 각 크기 7B의 전문가 64개를 가질 수 있으면서(그래서 총 파라미터 수가 거대함) 토큰당 그중 2개만 돌린다(그래서 토큰당 연산이 밀집 7B 모델과 일치함). Mixtral 8x7B는 47B의 총 파라미터를 가지지만 토큰당 13B만 활성화한다. DeepSeek-V3은 671B의 총 파라미터를 가지지만 토큰당 37B만 활성화한다.
...
스크립트에 묶여 있는 Llama 3 8B, Mistral 7B, Mixtral 8x7B, DeepSeek V3 설정에서 계산기를 실행하라. 파라미터 분해를 비교하라. MoE 모델은 밀집 모델을 압도하는 총 파라미터 수를 갖지만 종종 더 작은 활성 파라미터 수를 갖는다는 것에 주목하라. DeepSeek V3의 KV 캐시가 더 많은 총 파라미터를 가짐에도 Llama 3 405B의 것보다 작다는 것에 주목하라 — 그것이 MLA의 작동이다.
