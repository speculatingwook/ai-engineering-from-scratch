...
> 비디오는 사진의 더미가 아니다. 5초 클립에는 이미지 모델이 표현할 수 없는 인과 순서, 동작 동사, 사건 타이밍이 있다. Video-LLaMA(Zhang et al., 2023년 6월)는 오디오-비주얼 그라운딩(grounding)을 갖춘 최초의 오픈 비디오-LLM을 출시했다. VideoChat과 Video-LLaVA가 그 패턴을 확장했다. 2025년까지 Qwen2.5-VL의 TMRoPE가 프런티어 독점 모델과의 격차를 좁혔다. 각 시스템은 시간 토큰을 다르게 풀었다 — 클립당 Q-former, 프레임당 concat-pool, 토큰당 TMRoPE. 이 레슨은 그 패턴들을 읽고, 균일 대 동적 프레임 샘플러를 만들며, 시간 그라운딩 태스크에서 평가한다.
...
30 FPS의 1분 비디오는 1800프레임이다. 프레임당 196개 시각 토큰(224에서 ViT-B)이면 35만 2천 토큰으로 — 2024년 시대의 어느 LLM 컨텍스트보다도 크다.
...
TMRoPE는 "고양이가 몇 초에 뛰는가?" 질의를 가능하게 한다. 모델은 "4.2초에"를 출력할 수 있다. Video-LLaMA는 "클립 초반에"라고만 말할 수 있었다.
...
완전한 비디오-VLM 평가는 넷 모두를 친다. 이들은 서로 다른 축을 압박한다 — TempCompass는 순서가 전부이고, EgoSchema는 3분 이상의 추론에 관한 것이며, VideoMME는 여러 지속 시간에 걸친다.
