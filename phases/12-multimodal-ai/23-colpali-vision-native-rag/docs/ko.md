...
> 전통적인 RAG는 PDF를 텍스트로 파싱하고, 청크(chunk)로 분할하고, 청크를 임베딩(embedding)하고, 벡터(vector)를 저장한다. 모든 단계가 신호를 잃는다: OCR은 차트 데이터를 떨어뜨리고, 청킹은 표 행을 끊고, 텍스트 임베딩은 그림을 무시한다. ColPali(Faysse et al., 2024년 7월)는 더 단순한 질문을 던졌다: 왜 텍스트를 추출하는가? PaliGemma를 통해 페이지 이미지를 직접 임베딩하고, 검색에는 ColBERT 스타일의 지연 상호작용(late interaction)을 사용하며, 문서가 담은 모든 레이아웃, 그림, 폰트, 서식 신호를 유지하라. 발표된 벤치마크(benchmark): 시각적으로 풍부한 문서에서 텍스트-RAG보다 종단간 정확도가 20~40% 더 높다. ColQwen2, ColSmol, VisRAG가 이 패턴을 확장했다. 이 레슨은 비전 네이티브 RAG 명제를 읽고 작은 ColPali 같은 인덱서를 만든다.
...
PDF에 대한 텍스트-RAG는 문서의 대부분을 버린다. 재무 보고서의 3분기 매출 성장은 보통 차트에 있고, 의료 보고서의 소견은 주석이 달린 이미지에 있으며, 법률 계약의 서명란은 텍스트 사실이 아니라 레이아웃 사실이다.
...
ColPali의 해법: OCR을 건너뛰고, 페이지 이미지를 직접 임베딩한다. 검색에 ColBERT 스타일의 지연 상호작용을 사용하여 모델이 질의 시점에 세밀한 패치에 어텐션(attention)할 수 있게 한다.
...
장점: 강한 회상, 항(term) 수준 의미를 처리. 단점: 문서당 N_d개 벡터, 저장 비용이 비쌈.

### ColPali

ColPali(Faysse et al., arXiv:2407.01449)는 ColBERT 패턴을 이미지에 적용한다.

- 각 페이지는 PaliGemma(ViT + 언어)에 의해 패치 임베딩으로 인코딩된다: 페이지당 N_p개 벡터.
- 각 사용자 질의(텍스트)는 질의 토큰 임베딩으로 인코딩된다: N_q개 벡터.
- 점수 = Σ_i max_j cos(q_i, p_j), 즉 질의 텍스트 토큰과 페이지 이미지 패치에 대한 MaxSim.
- 총 점수로 상위 k개 페이지를 검색한다.

문서 입력 시점: 모든 페이지를 PaliGemma로 임베딩하고, 모든 패치 임베딩을 저장한다. 질의 시점: 질의 토큰을 임베딩하고, 저장된 모든 페이지 임베딩에 대해 MaxSim을 계산하고, 상위 k개 페이지를 반환한다.

장점: 시각적으로 풍부한 문서에서 종단간으로 텍스트-RAG를 20~40% 이긴다. 각 패치 벡터가 국소 레이아웃과 내용을 포착한다.

단점: 페이지당 N_p개 패치 × 4바이트 부동소수점 × D차원 벡터 = 저장이 빠르게 늘어난다. PQ / OPQ 양자화로 완화된다.

### ColQwen2와 ColSmol

ColQwen2(illuin-tech, 2024~2025)는 PaliGemma를 Qwen2-VL로 교체한다. 더 나은 기반 인코더, 더 나은 검색.

ColSmol은 로컬 / 엣지 사용을 위한 더 작은 규모의 변형이다. 약 1B 파라미터(parameter)의 ColSmol 검색기는 소비자용 GPU에서 실행된다.

### VisRAG

VisRAG(Yu et al., arXiv:2410.10594)는 다른 변형이다: 패치에 대한 MaxSim 대신, 각 페이지를 VLM으로 하나의 벡터로 풀링(pooling)한 다음 이중 인코더로 검색한다. 더 빠른 인덱싱 + 더 작은 저장, 더 약한 회상.
...
ColPali는 문서당 저장이 약 30배 많다. 규모에서, OPQ / PQ가 이를 약 5~10배로 낮추며, 대개 견딜 만하다.
...
- 장난감 페이지 5개를 인덱싱하고, 질의 3개를 실행하고, 점수와 함께 상위 k개를 반환한다.
