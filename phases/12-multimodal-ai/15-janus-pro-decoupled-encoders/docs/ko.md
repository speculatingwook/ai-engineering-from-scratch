...
> 통합 멀티모달 모델에는 피할 수 없는 긴장이 있다. 이해(understanding)는 의미 특성(semantic feature)을 원한다 — SigLIP이나 DINOv2는 개념 수준 정보가 풍부한 벡터를 낸다. 생성(generation)은 재구성에 친화적인 코드를 원한다 — 다시 선명한 픽셀로 합쳐지는 VQ 토큰. 두 목표는 단일 인코더(encoder)에서 양립하지 않는다. Janus(DeepSeek, 2024년 10월)와 Janus-Pro(DeepSeek, 2025년 1월)는 그 해법이 시도를 멈추는 것이라 주장한다. 두 인코더를 분리하라. 태스크 간에 트랜스포머(transformer) 본체를 공유하되, 이해는 SigLIP을 통해, 생성은 VQ 토크나이저(tokenizer)를 통해 라우팅한다. 7B에서 Janus-Pro는 GenEval에서 DALL-E 3을 이기면서 MMMU에서 LLaVA와 대등하다. 이 레슨은 왜 하나로는 실패하는 곳에서 두 인코더가 작동하는지 읽는다.
...
입력은 프롬프트(prompt) 형식으로 모호함이 해소된다. `<understand>` 태그는 SigLIP을 통해 라우팅하고, `<generate>`는 VQ를 통해 라우팅한다. 또는 라우팅이 태스크로부터 암묵적으로 정해진다.
...
공유 트랜스포머 본체는 두 입력 분포(SigLIP과 VQ)를 보고 둘 다와 작동하는 법을 배운다. 주장: 충분한 데이터 + 충분한 파라미터(parameter)면 본체가 전환을 흡수한다.
...
JanusFlow(arXiv 2411.07975)는 VQ 생성 경로를 정류 플로우(rectified-flow) 생성 경로(연속)로 교체한다. 분할이 이해용 SigLIP + 생성용 정류 플로우가 된다. 품질 한계가 더 올라간다. 아키텍처는 분리 인코더-공유 본체로 남는다.

### 공유 본체의 역할

트랜스포머 본체는 두 입력 분포를 가진 통합 시퀀스를 처리한다. 그 역할은:

- 이해를 위해: SigLIP 특성 + 텍스트 토큰을 소비 → 텍스트를 자기회귀(autoregressive)로 낸다.
- 생성을 위해: 텍스트 토큰 + (선택적 이미지 VQ 토큰)을 소비 → 이미지 VQ 토큰을 자기회귀로 낸다.
