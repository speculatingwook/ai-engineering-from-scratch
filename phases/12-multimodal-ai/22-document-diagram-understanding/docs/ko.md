...
> 문서는 사진이 아니다. PDF, 과학 논문, 청구서, 또는 손으로 쓴 양식에는 레이아웃, 표, 다이어그램, 각주, 머리글, 그리고 일반적인 이미지 이해가 포착할 수 없는 의미 구조가 있다. VLM 이전의 스택은 파이프라인(pipeline)이었다: Tesseract OCR + LayoutLMv3 + 표 추출 휴리스틱. VLM 물결은 그것을 OCR 없는 모델 — Donut(2022), Nougat(2023), DocLLM(2023) — 로 대체했으며, 이들은 구조화된 마크업을 직접 내보낸다. 2026년에 이르러 프런티어는 그저 "페이지 이미지를 2576px 네이티브로 Claude Opus 4.7에 공급"하는 것이고, 구조화된 마크업 출력은 덤으로 따라온다. 이 레슨은 문서 AI의 3시대 궤적을 읽는다.
...
원시 OCR은 텍스트를 덤프하고 나머지를 잃는다. 청구서를 신경 쓰는 시스템은 "합계: $1,245"가 각주가 아니라 오른쪽 아래에서 왔다는 것을 알아야 한다.
...
TrOCR(Li et al., arXiv:2109.10282)는 Tesseract의 고전적 CNN-CTC를 합성 + 실제 텍스트 이미지로 학습된 트랜스포머(transformer) 인코더-디코더로 대체했다. 손글씨와 다국어 텍스트에서 깔끔한 승리. 여전히 파이프라인(검출기 후 TrOCR 후 레이아웃)이지만, OCR 단계가 극적으로 개선되었다.
...
다른 트랙. LayoutLMv3(Huang et al., arXiv:2204.08387)는 OCR을 유지하되 레이아웃 이해를 더한다:

- 세 입력 스트림: OCR 텍스트 토큰(token), 토큰별 2D 경계 상자, 이미지 패치.
- 세 가지 모달리티 전반에 걸친 마스킹 학습 목표(마스킹된 텍스트, 마스킹된 패치, 마스킹된 레이아웃).
...
- PaliGemma 2(2025년 4월)는 문서 + 손글씨에 특화되어 학습된다.
...
2576픽셀 네이티브 입력에서, 프런티어 VLM은 인간에 가까운 정확도로 문서 이해를 한다. 2026년 초의 벤치마크 수치:
...
과학 논문은 수식에 대한 정확한 LaTeX 출력이 필요하다. Nougat은 이에 대해 학습되었다. LaTeX 목표로 학습된 VLM(Qwen2.5-VL-Math, Nougat 파생물)은 사용 가능한 LaTeX를 생성한다. 명시적 LaTeX 학습 없이는, VLM은 읽을 수 있지만 부정확한 전사(transcription)를 생성한다.
