...
BLIP-2는 32개의 시각 토큰을 동결된 LLM의 입력 층에 먹인다. 프롬프트당 이미지 하나에는 동작한다. 하지만 "여기 이미지 A가 있다, 캡션하라, 여기 이미지 B가 있다, 캡션하라, 이제 여기 이미지 C가 있다, 캡션하라"처럼 텍스트와 인터리브된 *많은* 이미지를 먹이고 싶다면? LLM의 셀프 어텐션(self-attention)은 단일 스트림에서 이미지 토큰과 텍스트 토큰을 처리해야 하고, 어떤 위치가 어떤 이미지에 주목할 수 있는지의 문제가 까다로워진다.
...
Flamingo가 답한 두 번째 질문: 프롬프트당 가변 개수의 이미지(0, 1, 또는 다수)를 어떻게 처리하는가? 퍼시버 리샘플러 — 당신이 가진 패치 개수가 무엇이든 받아서 고정 개수의 시각 잠재 토큰을 만드는 작은 교차 어텐션 모듈이다. LLM 교차 어텐션 층은 프롬프트에 이미지가 몇 개든 상관없이 같은 형상을 본다.
