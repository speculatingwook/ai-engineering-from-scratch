...
CLIP 이전의 비전은 지도 학습이었다. 레이블된 데이터셋(ImageNet: 120만 장 이미지, 1000개 클래스)을 모으고, CNN을 학습시켜 출시한다. 레이블은 비싸고, 레이블은 레이블러들이 합의할 수 있는 것 쪽으로 편향되며, 레이블은 파인튜닝(fine-tuning) 없이는 새 작업으로 전이되지 않는다.

이미지-캡션 웹에는 느슨하게 레이블된 10억 개 이상의 쌍이 공짜로 있다. alt 텍스트 "공원에 있는 내 강아지 맥스(my dog Max in the park)"가 달린 골든 리트리버 사진은 지도 신호를 담고 있다. 텍스트가 이미지를 묘사한다. 질문은 이것이다. 이것을 유용한 학습으로 바꿀 수 있는가?

CLIP의 답: 이미지-캡션 쌍을 매칭 작업으로 다룬다. N개의 이미지와 N개의 캡션으로 이루어진 배치가 주어지면, N-1개의 방해물(distractor)에 맞서 각 이미지를 자신의 캡션에 매칭하는 법을 학습한다. 지도 신호는 "이 둘은 서로 어울린다, 이 N-1개는 아니다"이다. 클래스 레이블 없음. 사람의 주석(annotation) 없음. 오직 대조 손실뿐.
...
이것이 InfoNCE다. CE 안의 소프트맥스는 각 이미지가 배치 안의 다른 모든 캡션보다 자신의 캡션에 더 매칭되도록 강제한다. "음성(negatives)"은 배치의 다른 모든 항목이다. 더 큰 배치 = 더 많은 음성 = 더 강한 신호. CLIP은 배치 32k에서 학습했다. 규모가 중요하다.

### 온도 (Temperature)

`tau`는 소프트맥스의 날카로움을 제어한다. 낮은 tau → 날카로운 분포, 어려운 음성 마이닝(hard negative mining) 효과. 높은 tau → 부드러움, 모든 샘플이 기여한다. CLIP은 붕괴를 막기 위해 클리핑된 log(1/tau)를 학습한다. SigLIP 2는 초기 tau를 고정하고 대신 학습되는 편향(bias)을 쓴다.
...
`i == j`이면 `y_ij = 1`, 아니면 0이다. 각 쌍의 손실은 독립적이다. all-gather가 필요 없다. 각 GPU가 자신의 로컬 블록을 계산해 합산한다. SigLIP 2는 CLIP이라면 비례적으로 더 많은 통신이 필요할 곳에서 배치 32k-512k까지 저렴하게 스케일링한다.
...
각 템플릿을 텍스트 인코더로 임베딩한다. 당신의 이미지를 이미지 인코더로 임베딩한다. 코사인 유사도의 argmax = 예측 클래스. 목표 클래스에 대한 학습은 없다.

프롬프트 템플릿이 중요하다. CLIP 원논문은 클래스당 80개 템플릿(plain, artistic, photo, painting 등)을 사용해 임베딩을 평균냈다. ImageNet +3점. 현대의 사용은 보통 한두 개 템플릿을 고른다.
...
2026년 오픈 VLM에서 SigLIP 2 SO400m/14는 기본 비전 타워다. CLIP은 특정 LAION-2B 학습 분포가 당신의 쿼리 패턴과 맞는 순수 이미지-텍스트 검색에서 여전히 기본값으로 남는다.

### ALIGN, BASIC, OpenCLIP, EVA-CLIP

ALIGN(Google, 2021): CLIP과 같은 아이디어, 18억 쌍 규모, 90% 잡음. 잡음 섞인 데이터가 스케일링됨을 증명했다. OpenCLIP(LAION): LAION-400M / 2B에서의 CLIP 오픈 재현, 여러 규모, 대표적인 오픈 체크포인트. EVA-CLIP: 마스킹 이미지 모델링(masked image modeling)에서 초기화한다. VLM을 위한 강력한 백본. BASIC: Google의 CLIP+ALIGN 하이브리드. 모두 같은 계열, 데이터와 튜닝만 다르다.
...
1. 4개 쌍으로 이루어진 배치에 대해 InfoNCE를 손으로 구현하라. 4x4 유사도 행렬을 만들고, 소프트맥스를 돌리고, 대각선을 골라내어 교차 엔트로피를 계산하라. 당신의 Python 구현을 이 손계산과 대조하여 검증하라.
