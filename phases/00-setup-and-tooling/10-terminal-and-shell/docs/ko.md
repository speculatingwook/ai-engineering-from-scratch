...
당신은 어떤 에디터에서보다도 터미널에서 더 많은 시간을 보내게 된다. 훈련 실행, GPU 모니터링, 로그 추적, 원격 SSH 세션, 환경 관리. 모든 AI 워크플로가 셸을 거친다. 여기서 느리면 어디서나 느리다.
...
세 가지가 동시에 돌아간다. 터미널 하나로. 분리(detach)하고, 집에 가고, 다시 SSH로 접속하고, 다시 붙일(reattach) 수 있다. 훈련은 계속 돌아간다.
...
파이핑은 명령어들을 서로 연결한다. 이것이 로그를 처리하고, 출력을 필터링하고, 도구를 연쇄하는 방식이다. 끊임없이 사용하게 된다.
...
이것들을 당신의 `~/.bashrc`나 `~/.zshrc`에 추가하라.
...
| 용어 | 흔히 하는 말 | 실제 의미 |
|------|----------------|----------------------|
| 셸(Shell) | "터미널" | 당신의 명령을 해석하는 프로그램(bash, zsh, fish) |
