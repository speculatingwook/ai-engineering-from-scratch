...
> 당신이 쓰는 도구가 당신의 사고방식을 결정한다. 한 번 제대로 세팅해 두자.
...
당신은 이제 Python, TypeScript, Rust, Julia를 사용해 200개가 넘는 레슨에 걸쳐 AI 엔지니어링을 배우게 된다. 환경이 망가져 있으면, 모든 레슨이 학습이 아니라 도구와 싸우는 일이 되어 버린다.

대부분의 사람들은 환경 설정을 건너뛴다. 그러고는 임포트(import) 오류, 버전 충돌, 빠진 CUDA 드라이버를 디버깅하느라 몇 시간을 쓴다. 우리는 이걸 한 번에, 제대로 해 둘 것이다.
...
이제 당신의 환경은 이 강의의 모든 레슨에 사용할 준비가 되었다. 어디에서 무엇을 쓰게 되는지 정리하면 다음과 같다.
