...
> 텍스트만으로는 서툰 제어 신호다. ControlNet은 사전 학습된 확산(diffusion) 모델을 복제하여 깊이 맵, 포즈 골격, 끄적임(scribble), 또는 가장자리 이미지로 조종할 수 있게 한다. LoRA는 1천만 개의 파라미터(parameter)를 학습하여 2B 파라미터 모델을 파인튜닝(fine-tune)할 수 있게 한다. 둘이 합쳐져 Stable Diffusion을 장난감에서 모든 에이전시가 출시하는 2026년 이미지 파이프라인으로 바꿔놓았다.
...
모든 신호(포즈, 깊이, canny, 분할)에 대해 밑바닥부터 새 조건부 모델을 학습하는 것은 엄두를 낼 수 없다. 당신은 2.6B 파라미터 SDXL 백본(backbone)을 동결한 채로 두고, 조건화를 읽는 작은 측면 네트워크를 붙이고, 그것이 백본의 중간 특성(feature)을 살짝 미는 것을 원한다. 그것이 ControlNet이다.

당신은 또한 전체 모델을 재학습하지 않고 모델에게 새 개념(당신의 얼굴, 당신의 제품, 당신의 스타일)을 가르치고 싶다. 100배 작은 델타(delta)를 원한다. 그것이 LoRA — 기존 어텐션(attention) 가중치(weight)에 끼워 넣는 저랭크 어댑터(low-rank adapter) — 다.
...
조건화로 *이미지*를 받는 아주 작은 어댑터(텍스트와 함께). CLIP 이미지 인코더를 사용해 이미지 토큰을 생성하고, 텍스트 토큰과 함께 교차 어텐션(cross-attention)에 주입한다. 기저 모델당 약 20MB. LoRA 없이 "이 레퍼런스의 스타일로 이미지 생성"을 할 수 있게 한다.
...
1. **LoRA.** 사전 학습된 선형 층 `W`. 그것을 동결한다. `W + BA`가 목표 선형 층과 맞도록 저랭크 `B @ A`를 학습한다. `r = 1`이면 랭크-1 보정을 완벽하게 학습하기에 충분함을 보여준다.
...
2. **보통.** 두 목표 변환에 두 개의 별도 LoRA를 학습하라. 그것들을 함께 로드하고 가산적 상호작용을 보여라. 상호작용이 언제 선형성을 깨는가?
