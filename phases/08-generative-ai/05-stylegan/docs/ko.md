...
DCGAN은 전치 합성곱(transposed convolution) 스택을 통해 `z`를 이미지로 매핑한다. 문제는: `z`가 모든 것 — 포즈, 조명, 신원, 배경 — 을 함께 얽힌(entangled) 채로 제어한다는 점이다. `z`의 한 축을 따라 움직이면 넷 모두가 바뀐다. 표현(representation)이 그렇게 인수분해되지 않으므로 모델에게 "같은 사람, 다른 포즈"를 요청할 수 없다.

Karras et al. (2019, NVIDIA)이 제안했다: `z`를 합성곱 층에 직접 넣는 것을 멈춰라. 상수 `4×4×512` 텐서를 네트워크 입력으로 넣어라. `z ∈ Z → w ∈ W`로 매핑하는 8층 MLP를 학습하라. *적응형 인스턴스 정규화(adaptive instance normalization)*(AdaIN)를 통해 모든 해상도에서 `w`를 주입하라: 각 합성곱 특성 맵(feature map)을 정규화한 다음, `w`의 아핀 투영(affine projection)으로 스케일하고 이동하라. 확률적 세부(피부 모공, 머리카락 가닥)를 위해 층별 잡음(per-layer noise)을 더하라.

결과: `W`는 "고수준 스타일"(포즈, 신원)과 "미세 스타일"(조명, 색)에 대해 거의 직교하는 축을 가진다. 이미지 A의 `w`를 저해상도 수준에, 이미지 B의 `w`를 고해상도에 사용함으로써 두 이미지 사이에서 스타일을 교환할 수 있다. 이것이 편집, 도메인 간 양식화, 그리고 "StyleGAN-역변환(inversion)"이라는 연구 계열 전체를 해금했다.

## 개념 (The Concept)

![StyleGAN: 매핑 네트워크 + AdaIN + 층별 잡음](../assets/stylegan.svg)

**매핑 네트워크(Mapping network).** `f: Z → W`, 8층 MLP다. `Z = N(0, I)^512`. `W`는 가우시안이도록 강제되지 않는다 — 데이터에 적응한 형태를 학습한다.
...
2026년에 StyleGAN3은 (a) 높은 FPS의 좁은 도메인 사실적 묘사, (b) 퓨샷(few-shot) 도메인 적응(100장 이미지로 새 데이터셋에 학습, 매핑은 동결), (c) 역변환 기반 편집(실제 사진을 재구성하는 `w`를 찾은 다음 그 `w`를 편집)의 기본값으로 남아 있다. 개방형 도메인 텍스트-이미지에는 적합한 도구가 아니다 — 확산(diffusion)이 그것이다.

## 직접 만들기 (Build It)

`code/main.py`는 1차원에서 장난감 "style-GAN lite"를 구현한다: 매핑 MLP, 학습된 상수 벡터를 받아 `w`에서 유도된 스케일/바이어스로 변조하는 합성 함수, 그리고 층별 잡음. 이는 아핀 변조를 통해 `w`를 주입하는 것이 생성기 입력에 `z`를 연결하는 것과 같거나 더 낫다는 것을 보여준다.
...
4090에서 StyleGAN3은 1024² FFHQ 얼굴을 10 ms 미만에 생성한다 — `num_steps = 1`, VAE 디코드 없음, 교차 어텐션(cross-attention) 패스 없음. 프로덕션 관점에서 이것은 모든 이미지 생성기의 바닥(floor) 지연 시간(latency)이다. 같은 해상도에서 50스텝 SDXL + VAE-디코드 파이프라인은 약 3초다. 그것은 **300배 격차**이고, 좁은 도메인 제품(아바타 서비스, 신분증 문서 파이프라인, 스톡 얼굴 생성)에서는 총소유비용(TCO)으로 이긴다.
