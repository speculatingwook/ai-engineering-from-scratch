...
둘 다 생성 순서(generation-order) 문제를 겪었다. 픽셀과 토큰은 2D 격자(grid)에 배열되어 있지만, 자기회귀 모델은 이를 1D 래스터 순서(raster order)로 방문해야 한다. 초기 모서리 픽셀은 이미지가 결국 무엇이 될지 전혀 알지 못한다. 생성 품질은 GPT-텍스트보다 더 나쁘게 확장되었고, 동일 컴퓨트에서 확산 모델 품질에 도달한 적이 없었다.
...
Tian et al.은 VAR이 ImageNet의 FID에 대해 거듭제곱 법칙(power-law) 스케일링 곡선을 따름을 입증했다. GPT가 퍼플렉서티에 대해 그러하듯이 말이다. 파라미터나 컴퓨트를 두 배로 늘리면 오차가 안정적으로 절반이 된다. 이는 언어 모델만큼 깔끔하게 이러한 스케일링 거동을 보인 최초의 이미지 생성 모델이었다. 그 결과 VAR 스케일 예측은 아키텍처마다 경험적으로 추측하는 것이 아니라 컴퓨트로부터 예측 가능해진다.
