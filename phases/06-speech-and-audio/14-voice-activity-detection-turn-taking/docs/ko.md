...
> 모든 음성 에이전트(agent)는 두 가지 판단에 따라 살고 죽는다. 사용자가 지금 말하고 있는가, 그리고 다 말했는가? VAD가 첫 번째에 답한다. 턴 감지(turn-detection)(VAD + 침묵 행오버 + 의미적 엔드포인트 모델)가 두 번째에 답한다. 둘 중 하나라도 틀리면 비서는 사용자 말을 자르거나 결코 입을 다물지 않는다.
...
순진한 답(에너지 임계값)은 어떤 잡음에서도 — 교통, 키보드, 군중 웅성거림 — 실패한다. 2026년의 답: Silero VAD(오픈, 딥러닝) + 턴 감지 모델(의미적 엔드포인팅) + VAD에 보정된 침묵 행오버.
...
**2단: Silero VAD** (2020-2026, MIT). 100만 파라미터. 6000개 이상의 언어로 학습됨. 단일 CPU 스레드에서 30ms 청크당 약 1ms로 동작. 5% FPR에서 87.7% TPR. 오픈소스 기본값이다.
...
스트리밍 STT 모델은 룩어헤드(look-ahead) 지연이 있다(Kyutai STT-1B는 500ms, STT-2.6B는 2.5초). 보통이면 음성 종료 후 그만큼 전사를 기다려야 한다. 플러시 트릭: VAD가 음성 종료를 발동하면, **STT에 플러시 신호를 보내** 즉시 출력을 강제한다. STT는 약 4배속으로 처리하므로, 500ms 버퍼가 약 125ms에 끝난다.
