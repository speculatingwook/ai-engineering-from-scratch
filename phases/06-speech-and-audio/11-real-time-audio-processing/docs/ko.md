...
Moshi(Kyutai, 2024)는 풀 듀플렉스(full-duplex) 200ms를 기록했다. GPT-4o-realtime(2024)은 약 320ms를 기록한다. 2022년의 캐스케이드(cascaded) 파이프라인은 2500ms로 출시되었다. 이 10배 개선은 세 가지 기법에서 나왔다. (1) 모든 곳에서의 스트리밍, (2) 부분 결과를 사용하는 비동기 파이프라이닝, (3) 중단 가능한 생성.
...
**링 버퍼(Ring buffer).** 고정 크기 순환 버퍼다. 프로듀서 스레드는 새 프레임을 쓰고, 컨슈머 스레드는 읽는다. 핫 패스(hot path)에서의 할당을 막는다. 크기 ≈ 최대 지연 시간 × 샘플 레이트. 2초짜리 16kHz 링은 32,000 샘플이다.
