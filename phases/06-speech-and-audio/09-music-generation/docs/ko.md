...
Meta의 **MusicGen**(2023, MIT)과 여러 파생물: 텍스트/멜로디 임베딩(embedding)에 조건화하여, EnCodec 토큰(token)(32 kHz, 4개 코드북(codebook))을 자기회귀적(autoregressive)으로 예측하고, EnCodec으로 디코드한다. 300M - 3.3B 파라미터(parameter). 강한 베이스라인(baseline)이지만; 30초를 넘기면 어려워한다.
...
- **FAD(Fréchet Audio Distance).** VGGish 또는 PANNs 특성을 사용한 생성 대 실제 오디오 분포 사이의 임베딩 수준 거리. 낮을수록 좋다. MusicGen small: MusicCaps에서 4.5 FAD; SOTA ~3.0.
...
- **Warner Music 대 Suno 합의.** 5억 달러. WMG는 이제 Suno에서 AI-유사성(AI-likeness), 음악 권리, 사용자 생성 트랙에 대한 감독권을 가진다. Udio에 대한 UMG 합의도 유사하다.
...
VGGish 임베딩 거리를 계산한다. 장르 수준의 회귀(regression) 테스트에 유용하지만; 인간 청취자를 대체하지는 않는다.
...
- **저작권 세탁 프롬프트.** "Taylor Swift 스타일의 노래" — 상업용 Suno/Udio는 이제 이를 필터링하지만, 오픈 모델은 그러지 않는다. 당신만의 필터 목록을 추가하라.
