...
- **기만적 정렬 모니터링**: 도구적 추론 오용에 대한 자동 모니터링에 대한 명시적 약속.
...
- 위치 찾기(Locate): 당신이 신경 쓰는 모든 능력이 정책에서 찾을 수 있어야 한다. 찾을 수 없으면 정책은 그것을 커버하지 않는 것이다.
- 분류(Classify): 그것은 추적(완화 발동)인가 연구(추적되지만 발동하지 않음)인가? OpenAI는 이를 명명한다. Anthropic과 DeepMind는 자신만의 대응물을 가진다.
...
5. 특정 능력(당신의 선택)에 대해 세 정책을 비교하라. 어느 정책의 분류가 가장 엄격하고 어느 것이 가장 덜 엄격한지 명명하라. 원본 텍스트로 정당화하라.
