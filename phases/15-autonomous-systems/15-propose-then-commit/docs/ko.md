...
에이전트가 액션을 취한다. 사용자는 결정해야 한다: 승인할 것인가 말 것인가. 결정이 즉각적이면, 그것은 아마 검토가 아니다. 결정이 구조화되어 있으면, 그것은 느리지만 신뢰할 만하다. 엔지니어링 질문은 구조화된 검토를 어떻게 최소 저항 경로로 만드느냐다.

2023년 시대의 HITL 패턴은 동기식 프롬프트(prompt)였다: "에이전트가 본문 Y로 X에게 이메일을 보내려 한다 — 승인?" 사용자가 승인을 클릭한다. 모두가 시스템이 안전하다고 느낀다. 실제로 이 표면은 심하게 고무 도장된다: 사용자가 빠르게 승인하고, 승인이 거의 예측하지 못하며, 에이전트가 잘못되면 감사 추적(audit trail)은 사용자가 기억하지 못하는 긴 승인 이력을 보여준다.
...
멱등성 키가 없으면, 일시적 실패 후 재시도가 승인된 액션을 이중 실행할 수 있다. 구체적 예: 사용자가 "A에서 B로 100달러 이체"를 승인한다. 네트워크가 깜빡인다. 워크플로가 재시도한다. 사용자는 한 번 승인했지만 이체가 두 번 실행된다. 멱등성 키는 승인을 단일하고 고유한 부작용에 묶는다. 두 번째 실행은 무연산(no-op)이다.
...
HITL의 기본 UI("Approve" / "Reject" 버튼)는 진정한 검토 없는 빠른 승인을 생산한다. 문서화된 완화책: Approve 버튼이 활성화되기 전에 특정 질문에 대한 긍정적 답변을 요구하는 챌린지-앤-리스폰스 체크리스트. 구체적 형태:

- "이것이 어떤 리소스를 건드리는지 이해하는가? [ ]"
- "폭발 반경이 받아들일 만한지 검증했는가? [ ]"
- "이것이 실패하면 롤백 계획이 있는가? [ ]"

그 자체를 위한 관료주의가 아니라 — 강제 함수(forcing function)다. 박스를 체크할 수 없는 검토자는 명확화를 요청하거나(격상) 거절한다(안전한 기본값). Anthropic 에이전트 안전 연구는 고무 도장 승인 패턴에 대한 완화책으로 체크리스트 기반 HITL을 명시적으로 인용한다.
...
"커밋이 실행되었다"는 "부작용이 일어났다"와 같지 않다. 네트워크 분할(network-partition)과 경쟁 조건(race condition)은 백엔드가 지속하지 않았는데 성공했다고 여기는 워크플로를 만들 수 있다. 검증 단계는 커밋 후 대상 리소스를 다시 읽어 확인한다. 이것은 `RETURNING` 절을 갖춘 데이터베이스 트랜잭션이나 `PutObject` 후 AWS `GetObject`와 같은 패턴이다.
