...
당신의 vLLM 서빙은 동시성이 오를 때마다 선점(preemption) 이벤트와 함께 GPU가 100% HBM에 있는 것을 보여준다. 요청이 축출(evict)되고, 재큐잉되며, 같은 2K 토큰 프롬프트를 1분에 네 번 재프리필한다. GPU 연산이 중복 프리필에 쓰인다. 굿풋(goodput)이 원시 처리량보다 한참 아래다.
...
vLLM 0.9.0은 플러그형 KV 캐시 백엔드를 위한 Connector API를 도입했다. 엔진이 블록을 커넥터로 오프로드하고, 커넥터가 그것들을 저장한다(RAM, 디스크, 오브젝트 스토리지, LMCache). 요청이 블록을 필요로 하면, 커넥터가 그것을 다시 로드한다.

vLLM 0.11.0(2026년 1월)은 비동기 오프로드 경로를 추가한다 — 오프로드가 백그라운드에서 일어날 수 있어 일반적인 경우에 엔진이 그것에 블로킹되지 않는다. 종단 간 지연 시간과 처리량은 여전히 워크로드 형태, KV 캐시 적중률, 시스템 압박에 의존한다. vLLM 자체 노트는 커스텀 커널 오프로드가 낮은 적중률에서 처리량을 저하시킬 수 있고, 비동기 스케줄링이 추측 디코딩(speculative decoding)과 알려진 상호작용 문제가 있음을 지적한다.
