...
인턴이 API 키가 든 `.env`를 커밋한다. 빠르게 삭제한다. 키는 이미 git 히스토리에 있다 — GitGuardian 스캔이 잡아내고, 당신의 순환 절차는 "팀에 슬랙하고, 40개 설정 파일을 갱신하고, 모든 서비스를 재배포한다"이다. 8시간 후, 서비스의 절반은 라이브고 절반은 배포 윈도를 기다린다.
...
공급망 공격: 손상된 CI/CD 자격 증명이 수천 개 고객 배포에 걸쳐 env 변수를 유출했다. 교훈: CI/CD 자격 증명은 프로덕션과 동등하다. 볼트에 저장하라. 좁게 범위를 둬라. 공격적으로 순환하라.
