...
당신은 Llama 3.3 70B를 8개의 H100에서 돌린다. 혼합 워크로드(긴 프롬프트 + 짧은 출력)에서, 연산 대부분이 프리필에 쓰였기 때문에 디코드 동안 GPU가 유휴 상태가 된다. 다른 워크로드(짧은 프롬프트 + 긴 출력)에서는 반대가 일어난다. 코로케이션된 프리필 + 디코드는 둘 다 과다 프로비저닝함을 뜻한다.
...
이들을 코로케이션하면: 둘 다에 최적화된 GPU를 산다. H100은 둘 다 잘하지만 어느 쪽이든 같은 비용이 든다. 스케일에서는 프리필 풀을 H100 / 연산 중심에, 디코드 풀을 H200 / 메모리 중심에, 또는 공격적 양자화와 함께 두기를 원한다.
...
NIXL은 NVIDIA의 노드 간 전송이다. 가능할 때 RDMA/InfiniBand를 쓰고, 아니면 TCP 폴백을 쓴다. 전송 지연 시간은 실재한다 — 보통 70B FP8에서 4K 토큰 프롬프트의 KV 캐시에 대해 20-80ms. 이것이 짧은 프롬프트가 분리를 정당화하지 못하는 이유다: 전송 세금이 절감을 초과한다.
...
4. KV 전송 비용을 계산하라: 70B FP8에서 4K 프리필 = ~500 MB KV. RDMA 100 GB/s에서 전송 = 5ms. TCP 10 GB/s에서 = 50ms. 당신의 SLA에는 어느 것이 중요한가?
