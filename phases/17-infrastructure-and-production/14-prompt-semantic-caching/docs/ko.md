...
당신은 RAG 서비스에 프롬프트 캐싱을 추가한다. 청구액은 평평하게 유지된다. 적중률을 측정하니 7%다. 프롬프트가 정적으로 보이지만 그렇지 않다 — 시스템 프롬프트에는 분 단위로 포맷된 현재 날짜, 요청 ID, 다양성을 위해 무작위로 재배열된 예시가 들어 있다. 모든 요청이 새 캐시 엔트리를 쓰고, 읽기는 0이다.

별개로, 당신의 에이전트는 사용자 질문당 열 개의 병렬 도구 호출을 돌린다. 첫 캐시 쓰기가 완료되기 전에 열 개 모두 프로바이더에 도착한다. 열 번의 쓰기, 0번의 읽기. 당신의 청구액은 "캐싱 적용" 시 비용이 되어야 했던 것의 5-10배다.
...
당신의 에이전트가 10개의 도구 호출을 병렬로 한다. 10개 모두 같은 4K 토큰 시스템 프롬프트를 갖는다. Anthropic 캐시 쓰기는 요청별이다. 첫 캐시 쓰기는 프로바이더가 프롬프트를 본 뒤 약 300ms 후에 완료된다. 요청 2-10은 같은 밀리초 윈도우에 도착해 각각 캐시 미스를 본다. 10개의 쓰기 프리미엄을 지불하고, 0개의 읽기 할인을 받는다.

해결책: 순차 우선으로 배치하라 — 요청 1을 단독으로 보낸 뒤, 1의 캐시가 채워지면 2-10을 발사한다. 첫 도구 호출에 300ms를 더하지만, 청구액의 5-10배를 절약한다.

### 동적 콘텐츠 안티패턴

당신의 시스템 프롬프트가 이렇게 생겼다:
...
해결책: 진정으로 정적인 모든 것을 캐시 가능 프리픽스로 옮기고, 동적 콘텐츠는 캐시 경계 뒤에 덧붙인다:
...
2. 시스템 프롬프트에 날짜가 있다. 그것을 밖으로 옮겨라. 전후 적중률 산수를 보여라.
3. 당신의 요청 도착률이 주어졌을 때, 1시간 TTL(2배 쓰기) 대 5분 TTL(1.25배 쓰기)의 손익분기점을 계산하라.
4. 0.95 임계값의 시맨틱 캐시가 20% 적중한다. 0.85에서는 50% 적중하지만 잘못된 캐시 응답이 보인다. 올바른 임계값을 고르고 정당화하라.
5. 사용자 질문당 10개의 병렬 서브 쿼리를 배치한다. 종단 간 지연 시간을 더하지 않으면서 캐시 친화적으로 다시 작성하라.
