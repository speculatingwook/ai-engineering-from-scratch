...
> 비선형성(nonlinearity)이 없으면, 당신의 100층짜리 신경망은 거창한 행렬 곱일 뿐이다. 활성화 함수(activation function)는 신경망이 곡선으로 사고하게 해 주는 관문이다.
...
두 개의 선형 변환을 쌓아 보자: y = W2(W1x + b1) + b2. 전개하면: y = W2W1x + W2b1 + b2. 이것은 그저 y = Ax + c -- 하나의 선형 변환일 뿐이다. 선형 층을 몇 개를 쌓든, 결과는 하나의 행렬 곱으로 붕괴한다. 당신의 100층짜리 신경망은 단 한 층과 똑같은 표현력을 가진다.

이것은 이론적 호기심이 아니다. 깊은 선형 신경망은 말 그대로 XOR를 학습할 수 없고, 나선(spiral) 데이터셋을 분류할 수 없으며, 얼굴을 인식할 수 없다는 뜻이다. 활성화 함수가 없으면, 깊이는 환상이다.

활성화 함수는 선형성을 깨뜨린다. 각 층의 출력을 비선형 함수로 휘게 하여, 신경망에게 결정 경계(decision boundary)를 구부리고, 임의의 함수를 근사하고, 실제로 학습하는 능력을 준다. 하지만 잘못된 활성화를 고르면 그래디언트(gradient)가 0으로 소실되거나(깊은 신경망에서의 시그모이드), 무한대로 폭발하거나(세심한 초기화 없는 비유계 활성화), 뉴런이 영구히 죽는다(큰 음수 편향을 가진 ReLU). 활성화 함수의 선택이 신경망이 학습을 하느냐 마느냐를 직접 결정한다.

## 개념 (The Concept)

### 왜 비선형성이 필요한가

행렬 곱(matrix multiplication)은 조합 가능(composable)하다. 벡터에 행렬 A를 곱한 뒤 행렬 B를 곱하는 것은 AB를 곱하는 것과 동일하다. 이것은 선형 층 열 개를 쌓는 것이 하나의 큰 행렬을 가진 하나의 선형 층과 수학적으로 동등하다는 뜻이다. 그 모든 파라미터(parameter), 그 모든 깊이가 -- 낭비된다. 그 연쇄(chain)를 깨뜨릴 무언가가 필요하다. 그것이 활성화 함수가 하는 일이다.
...
이제 대입이 깨진다. W2 * g(W1 * x + b1) + b2는 하나의 선형 변환으로 환원될 수 없다. 신경망은 비선형 함수를 표현할 수 있다. 활성화를 가진 층이 하나씩 더해질 때마다 표현 용량이 늘어난다.
...
원래 신호의 백만분의 일도 안 된다. 이것이 기울기 소실(vanishing gradient) 문제다. 앞쪽 층의 그래디언트가 너무 작아져서 가중치(weight)가 거의 갱신되지 않는다. 신경망은 학습하는 것처럼 보인다 -- 뒤쪽 층에서는 손실(loss)이 줄어든다 -- 하지만 첫 번째 층들은 얼어붙어 있다. 깊은 시그모이드 신경망은 그냥 학습이 안 된다.

추가 문제: 시그모이드 출력은 항상 양수(0에서 1)이며, 이는 가중치에 대한 그래디언트가 항상 같은 부호임을 뜻한다. 이로 인해 경사 하강법(gradient descent) 중에 지그재그가 발생한다.
...
최대 도함수는 x = 0에서 1.0이다 -- 시그모이드보다 네 배 낫다. 하지만 기울기 소실 문제는 여전히 존재한다. 큰 양수나 음수 입력에 대해, 도함수는 0에 가까워진다. 열 층은 여전히 그래디언트를 짓뭉갠다. 다만 덜 공격적일 뿐이다.

### ReLU: 돌파구

정류 선형 유닛(Rectified Linear Unit). 2010년 Nair와 Hinton에 의해 딥러닝용으로 대중화되었고(함수 자체는 1969년 Fukushima의 연구로 거슬러 올라간다), 모든 것을 바꿨다.
...
양수 입력에 대해서는 기울기 소실이 없다. 그래디언트는 정확히 1이고, 곧장 통과한다. 이것이 깊은 신경망이 학습 가능해진 이유다 -- ReLU는 층을 거쳐도 그래디언트 크기를 보존한다.

하지만 실패 모드가 있다: 죽은 뉴런(dead neuron) 문제다. 어떤 뉴런의 가중 입력이 항상 음수라면(큰 음수 편향이나 불운한 가중치 초기화 때문에), 그 출력은 항상 0이고, 그래디언트는 항상 0이며, 절대 갱신되지 않는다. 영구히 죽은 것이다. 실제로, ReLU 신경망에서 뉴런의 10-40%가 학습 중에 죽을 수 있다.
...
GELU는 모든 곳에서 매끄럽고, 작은 음수 값을 허용하며(0으로 딱딱하게 잘라 버리는 ReLU와 달리), 확률적 해석을 가진다: 각 입력을 가우시안 분포 하에서 그것이 양수일 확률에 따라 가중한다. 이 매끄러운 게이팅(gating)은 트랜스포머 아키텍처에서 ReLU를 능가하는데, 더 나은 그래디언트 흐름을 제공하고 죽은 뉴런 문제를 완전히 피하기 때문이다.
...
Swish는 형식적으로 x * sigmoid(x)다. Google은 이를 활성화 함수 공간에 대한 자동 탐색을 통해 발견했다 -- 신경망이 신경망의 일부를 설계한 것이다.

GELU처럼 매끄럽고, 비단조(non-monotonic)하며, 작은 음수 값을 허용한다. 차이는 미묘하다: Swish는 게이팅에 시그모이드를 쓰고 GELU는 가우시안 CDF를 쓴다. 실제로는 성능이 거의 동일하다. Swish는 EfficientNet과 일부 비전 모델에서 사용된다. GELU는 언어 모델에서 지배적이다.
...
모든 출력은 0과 1 사이다. 모든 출력의 합은 1이다. 이 때문에 다중 클래스 분류(classification)의 표준 최종 활성화가 된다. 가장 큰 로짓이 가장 높은 확률을 받지만, argmax와 달리 소프트맥스는 미분 가능하며 상대적 확신에 대한 정보를 보존한다.
...
트랜스포머의 은닉층: GELU. CNN의 은닉층: ReLU. 분류용 출력층: 소프트맥스. 회귀(regression)용 출력층: 없음(선형). 확률용 출력층: 시그모이드. 그게 전부다. 이 기본값으로 시작하라. 근거가 있을 때만 바꿔라.
