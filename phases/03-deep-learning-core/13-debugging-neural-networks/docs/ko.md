...
> 당신의 신경망(neural network)은 컴파일되었다. 실행되었다. 숫자를 하나 내놓았다. 그 숫자는 틀렸는데 아무것도 크래시하지 않았다. 가장 어려운 종류의 디버깅 — 오류 메시지가 없는 종류 — 에 온 것을 환영한다.
...
고장 난 신경망은 끝까지 실행되고, 손실(loss) 값을 출력하고, 예측을 내놓는다. 손실은 감소할 수도 있다. 예측은 그럴듯해 보일 수도 있다. 하지만 모델은 조용히 틀려 있다 — 지름길을 학습하거나, 잡음을 암기하거나, 쓸모없는 국소 최솟값(local minimum)으로 수렴(convergence)한다. Google 연구자들은 ML 디버깅 시간의 60-70%가 오류를 내지 않으면서 모델 품질을 떨어뜨리는 "조용한(silent)" 버그에 쓰인다고 추정했다.

작동하는 모델과 고장 난 모델의 차이는 종종 잘못 놓인 단 한 줄이다: 빠진 `zero_grad()`, 전치된 차원, 10배 어긋난 학습률. 전형적인 "Recipe for Training Neural Networks" (2019)는 이렇게 시작한다: "가장 흔한 신경망 실수는 크래시하지 않는 버그다."
...
print하고 기도하는 디버깅은 잊어라. 신경망 디버깅은 체계적인 접근이 필요한데, 피드백 루프가 느리고(학습 실행당 수 분에서 수 시간) 증상이 모호하기 때문이다(나쁜 손실은 20가지 다른 것을 의미할 수 있다).
...
**죽은 ReLU(Dead ReLUs).** ReLU 뉴런이 큰 음수 입력을 받으면 0을 출력하고 그래디언트도 0이다. 다시는 활성화되지 않는다. 충분히 많은 뉴런이 죽으면 네트워크는 학습할 수 없다. 확인: 각 ReLU 층(layer) 뒤에서 정확히 0인 활성값의 비율을 출력하라. 50% 이상이 죽었다면 LeakyReLU로 전환하거나 학습률을 낮춰라.
...
**그래디언트 폭발(Exploding gradients).** 반대 문제 — 그래디언트가 지수적으로 커진다. RNN과 아주 깊은 네트워크에서 흔하다. 손실이 NaN으로 뛴다. 해결책: 그래디언트 클리핑(gradient clipping)(`torch.nn.utils.clip_grad_norm_`), 학습률 낮추기, 또는 정규화(normalization) 추가.
...
작은 배치 하나(8-32 샘플)를 가져온다. 그것에 대해 100회 이상 반복하며 학습한다. 손실은 거의 0으로 가야 하고 학습 정확도는 100%에 도달해야 한다. 그렇지 않으면, 모델이나 학습 루프에 근본적인 버그가 있다 — 전체 학습으로 진행하지 마라.
