...
> 누군가는 백만 GPU 시간을 들여 신경망(neural network)에게 가장자리, 질감, 객체 부분이 어떻게 생겼는지 가르쳤다. 당신은 자신의 학습을 시작하기 전에 그 특성들을 빌려야 한다.
...
ImageNet에서 ResNet-50을 학습시키는 데는 약 2,000 GPU 시간이 든다. 자기들이 출고하는 모든 작업마다 그 예산을 가진 팀은 거의 없다. 거의 모든 팀이 실제로 출고하는 것은 수백 또는 수천 개의 작업 특화 이미지로 학습된 새 헤드를 가진 사전 학습된 백본이다.

이것은 지름길이 아니다. 모든 ImageNet 학습 CNN의 첫 합성곱(convolution) 블록은 가장자리와 가보(Gabor) 유사 필터를 학습한다. 다음 몇 블록은 질감과 단순한 모티프를 학습한다. 중간 블록은 객체 부분을 학습한다. 마지막 블록은 1,000개 ImageNet 범주처럼 보이기 시작하는 조합을 학습한다. 그 위계의 첫 90%는 의료 영상, 산업 검사, 위성 데이터, 그리고 다른 모든 비전 작업에 거의 변경 없이 전이된다 — 자연은 가장자리와 질감의 어휘가 한정되어 있기 때문이다. 마지막 10%가 당신이 실제로 학습시키는 것이다.

전이를 제대로 하는 데는 당신을 기다리는 세 가지 버그가 있다. 너무 높은 학습률로 사전 학습된 특성을 파괴하는 것, 너무 많이 고정하여 모델을 정보에 굶주리게 하는 것, 그리고 나머지 신경망이 한 번도 학습하지 않은 작은 데이터셋 쪽으로 BatchNorm의 실행 통계가 표류하게 두는 것이다. 이 레슨은 그 각각을 일부러 거쳐 간다.
...
BN 층은 ImageNet에서 계산된 `running_mean`과 `running_var` 버퍼를 갖는다. 당신의 작업이 다른 픽셀 분포 — 다른 조명, 다른 센서, 다른 색 공간 — 를 가지면, 그 버퍼는 틀렸다. 선호 순서대로 세 가지 선택지:
...
분류기 헤드는 선형 층 1-3개에 선택적 드롭아웃을 더한 것이다. 모든 torchvision 백본은 당신이 교체하는 기본 헤드를 함께 출고한다.
...
- **사전 학습만의 정확도** — 백본을 고정한 헤드의 정확도. 이것이 당신의 바닥이다.
- **파인튜닝된 정확도** — 종단 간 학습 후의 같은 모델. 이것이 당신의 천장이다.
