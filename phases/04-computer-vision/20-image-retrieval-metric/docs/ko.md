...
> 검색 시스템은 임베딩(embedding) 공간에서의 거리를 기준으로 후보를 순위 매긴다. 메트릭 학습(metric learning)은 그 거리가 당신이 원하는 의미를 갖도록 공간을 빚어내는 분야다.
...
두 가지 설계 결정이 시스템 전체를 결정한다. 임베딩 — 어떤 모델(model)이 벡터(vector)를 만드는가. 인덱스 — 대규모에서 최근접 이웃(nearest neighbour)을 어떻게 찾는가. 2026년에는 둘 다 범용 부품이 되었고(임베딩에는 DINOv2, 인덱스에는 FAISS), 그래서 기준이 높아졌다. 어려운 부분은 당신의 애플리케이션에서 *무엇이 유사하다고 간주되는지*를 정의하고, 그 거리가 일치하도록 임베딩 공간을 빚어내는 것이다.
...
대부분의 프로덕션 사용 사례에서는, 사전 학습된(pretrained) 백본으로 시작하고, 기성품 임베딩이 당신의 테스트 집합에서 성능이 떨어질 때만 메트릭 학습 파인튜닝(fine-tune)을 추가한다.
...
대부분의 현대 신경망에서 둘은 동등하다. `||a|| = ||b|| = 1`일 때 `||a - b||^2 = 2 - 2 cos(a, b)`이기 때문이다. 당신의 임베딩 학습(training)에 맞는 관례를 골라라. 둘을 섞으면 "가장 가까운"이 의미하는 바가 조용히 바뀐다.
...
recall@1, @5, @10을 나란히 보고한다. recall@10이 0.95를 넘는데 recall@1이 0.5 미만이라면, 임베딩 공간은 올바른 구조를 가졌지만 순위 매기기가 노이즈가 많다는 뜻이다 — 더 긴 파인튜닝이나 재순위(re-ranking) 단계를 시도해 보라.
