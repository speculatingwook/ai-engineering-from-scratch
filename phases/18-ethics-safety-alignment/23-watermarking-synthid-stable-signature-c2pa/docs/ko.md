...
- 토큰 수준 워터마킹(SynthID-text 방식)과 그것이 탐지 가능해지는 메커니즘을 기술하기.
- Stable Signature와, 그것을 무력화한 2024년 제거 공격을 기술하기.
- C2PA의 역할과 그것이 왜 워터마킹에 상호 보완적인지 진술하기.
...
Google에 의해 프로덕션화된 Kirchenbauer et al. 2023 메커니즘:
...
Fernandez et al. ICCV 2023. 잠재 확산 디코더를 파인튜닝하여, 모든 생성 이미지가 잠재 표현(latent representation)에 내장된 고정 이진 메시지를 담도록 한다. 탐지는 신경 디코더로 잠재로부터 디코딩된다. (콘텐츠의 10%로) 크롭된 이미지가 FPR<1e-6에서 90% 초과로 탐지됨.
