...
레드팀은 한때 수작업 활동이었다. 소수의 전문 테스터가 적대적 프롬프트를 구성하고 어떤 것이 통하는지 추적했다. 이것은 확장되지 않는다: 공격 성공률에는 통계적 표본이 필요하고, 대상은 모델이 출시될 때마다 움직이는 표적이다. PAIR는 레드팀을 블랙박스 대상을 가진 최적화 문제로 조작화한다.
...
GCG(Zou 외 2023)는 그래디언트(gradient)로 적대적 토큰 접미사(suffix)를 탐색한다; 화이트박스 모델 접근이 필요하고 읽을 수 없는 접미사를 만든다. PAIR는 블랙박스이며 모델 간에 전이(transfer)되는 자연어 공격을 만든다. PAIR의 인컨텍스트 피드백은 공격자가 매 거부로부터 학습하게 한다; GCG에는 이에 상응하는 것이 없다(매 토큰 갱신이 이전 진척을 처음부터 다시 발견해야 한다).
...
이제 모든 프런티어(frontier) 연구소는 출시 전 프로덕션 모델에 대해 PAIR와 TAP를 실행한다. ASR 궤적은 모델 카드(model card, Lesson 26)와 안전 사례(safety-case) 부록(Lesson 18)에 등장한다. 이 공격은 별난 것이 아니다 — 표준 인프라다.
