...
모델은 패턴을 이어간다. 컨텍스트 안의 어시스턴트 턴은 가짜다 — 대상 모델이 실제로 내놓은 적이 없다 — 하지만 대상은 그것을 따라야 할 패턴으로 취급한다.
...
거듭제곱 법칙 — 로지스틱(logistic)이 아니다. 샷을 늘려도 정체되지 않는다; 계속 올라간다.

### ICL과 메커니즘을 공유하는 이유

양성 ICL: 모델은 인컨텍스트 예시로부터 작업을 추출하고 쿼리에 대해 그것을 실행한다. MSJ: 모델은 인컨텍스트 예시로부터 "유해한 요청에 응하기"를 추출하고 대상에 대해 실행한다.

거듭제곱 법칙 형태는 동일하다. 모델은 둘을 구분하지 못하는데, 메커니즘 — 인컨텍스트 예시로부터의 패턴 추출 — 이 같기 때문이다.
...
MSJ는 PAIR(Lesson 12)와 합성된다: PAIR로 공격 구조를 찾고, 거기에 다중 샷을 채운다. Anil 외 2024(Anthropic)는 MSJ가 경쟁 목적(competing-objective) 탈옥과 합성됨을 보고한다 — 쌓아 올리면 어느 한쪽 단독보다 더 높은 ASR에 도달한다.
...
5. MSJ의 메커니즘은 ICL과 동일하다. 양성 작업 패턴에 대한 ICL 민감도를 줄이지 않으면서 유해-응순 패턴에 대한 ICL 민감도를 줄이는 학습 시점(training-time) 방어를 스케치하라. 당신 설계의 주요 실패 모드를 식별하라.
