...
> MCP는 "어떤 툴이 존재하는가"를 말한다. 스킬(skill)은 "태스크를 어떻게 하는가"를 말한다. 2026년 스택은 둘 다 계층화한다. Anthropic의 Agent Skills(오픈 표준, 2025년 12월)는 점진적 공개(progressive disclosure)를 갖춘 SKILL.md로 출하된다. OpenAI의 Apps SDK는 MCP에 위젯 메타데이터를 더한 것이다. AGENTS.md(현재 60,000개 이상의 레포에 있음)는 프로젝트 수준 에이전트 컨텍스트로 레포 루트에 자리한다. 이 레슨은 각각이 무엇을 다루는지를 명명하고, 에이전트들을 가로질러 이동하는 최소 SKILL.md + AGENTS.md 번들을 구축한다.
...
어느 엔지니어가 릴리스 노트 작성 워크플로를 다단계 프롬프트로 증류한다: "최근 머지된 PR을 읽어라. 영역별로 그룹화하라. 각각을 요약하라. 팀의 스타일을 따르는 체인지로그 항목을 작성하라. Slack 초안에 게시하라." 그들은 이것을 팀을 위해 Notion 문서에 넣는다.

이제 그들은 이 워크플로를 Claude Code, Cursor, Codex CLI에서 사용하고 싶다. 각 에이전트는 명령을 로드하는 방식이 다르다: Claude Code 슬래시 명령, Cursor 규칙, Codex `.codex.md`. 엔지니어는 워크플로를 세 번 복사하고 세 사본을 유지한다.

AGENTS.md와 SKILL.md가 함께 이를 고친다:

- **AGENTS.md**는 레포 루트에 자리한다. 모든 호환 에이전트가 세션 시작 시 그것을 읽는다. "이 프로젝트는 어떻게 작동하는가? 컨벤션은 무엇인가? 어떤 명령이 테스트를 실행하는가?"
- **SKILL.md**는 이식 가능한 번들이다: YAML 프론트매터(name, description) + 마크다운 본문 + 선택적 리소스. 스킬을 지원하는 에이전트가 필요 시 이름으로 그것들을 로드한다.
...
2025년 말 출범했고, 2026년 4월까지 60,000개 이상의 레포가 채택했다. 레포 루트의 파일 하나. 형식:
...
에이전트는 세션 시작 시 이를 읽고 그 프로젝트에 맞게 자신의 행동을 보정하는 데 사용한다. 2026년의 모든 코딩 에이전트가 AGENTS.md를 지원한다: Claude Code, Cursor, Codex, Copilot Workspace, opencode, Windsurf, Zed.
...
프론트매터는 스킬의 정체성을 선언한다. 본문은 스킬이 로드될 때 모델에 보여지는 프롬프트다.
...
SKILL.md는 "스타일 규칙은 style-guide.md를 보라"고 말한다. 에이전트는 스킬이 활발히 실행될 때만 style-guide.md를 가져온다. 이는 모델이 필요로 하지 않을 수 있는 세부로 프롬프트를 부풀리는 것을 피한다.
...
`@anthropic-ai/claude-agent-sdk`(TypeScript)와 `claude-agent-sdk`(Python)는 세션 시작 시 스킬을 로드하고, 런타임 내부에서 호출 가능한 "에이전트"로 노출한다. 사용자가 스킬을 호출하면 에이전트 루프가 그것으로 디스패치한다.

### OpenAI Apps SDK

2025년 10월 출범; MCP 위에 직접 구축되었다. OpenAI의 이전 Connectors와 Custom GPT Actions를 단일 개발자 표면 아래 통합한다. Apps SDK 앱은 다음이다:

- MCP 서버(툴, 리소스, 프롬프트).
- 더하기 ChatGPT의 UI를 위한 위젯 메타데이터.
- 더하기 인터랙티브 표면을 위한 선택적 MCP Apps `ui://` 리소스.
...
셋 모두 결합한다: 에이전트가 세션 시작 시 AGENTS.md를 읽고, 사용자가 스킬을 호출하고, 스킬의 명령이 MCP 툴 호출을 포함하고, 에이전트가 MCP 클라이언트를 통해 디스패치한다.

## 라이브러리로 써보기 (Use It)

`code/main.py`는 stdlib SKILL.md 파서와 로더를 출하한다. `./skills/` 아래에서 스킬을 디스커버리하고, YAML 프론트매터와 마크다운 본문을 파싱하고, 스킬 이름을 키로 하는 딕셔너리를 만든다. 그런 다음 `release-notes-writer`를 이름으로 호출하는 에이전트 루프를 시뮬레이션한다.

볼 것:

- 최소 stdlib 파서로 파싱된 YAML 프론트매터(`pyyaml` 의존성 없음).
- 스킬 본문은 그대로 저장된다; 에이전트가 호출 시 그것을 시스템 프롬프트에 앞에 붙인다.
- 참조된 파일을 필요 시 가져오는 `read_subresource` 함수를 통해 시연되는 점진적 공개.

## 산출물 (Ship It)

이 레슨은 `outputs/skill-agent-bundle.md`를 만든다. 워크플로가 주어지면, 이 스킬은 에이전트들을 가로질러 이식 가능한 결합된 SKILL.md + AGENTS.md + MCP 서버 청사진 번들을 만들어낸다.
...
3. 당신 팀의 내부 문서에서 다단계 워크플로를 SKILL.md로 이식하라. Claude Code에서 로드되는지 검증하라.

4. 스킬을 Cursor와 Codex의 네이티브 규칙 형식으로 손으로 번역하라. 형식 간 차이를 세어라 — 이것이 SkillKit이 자동화하는 번역 표면이다.

5. Anthropic Agent Skills 블로그 글을 읽어라. 이 레슨의 로더가 다루지 않는 Claude Agent SDK의 기능 하나를 식별하라. (힌트: 에이전트 하위 호출.)
