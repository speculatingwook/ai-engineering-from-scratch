...
> 제공자 종속(provider lock-in)은 비싸다. 서로 다른 툴 호출 워크로드는 서로 다른 모델에 적합하다. 라우팅 게이트웨이(routing gateway)는 하나의 API 표면, 재시도, 페일오버(failover), 비용 추적, 가드레일(guardrail)을 제공한다. 2026년에는 세 가지 원형(archetype)이 지배한다: LiteLLM(오픈소스 셀프 호스팅), OpenRouter(매니지드 SaaS), Portkey(프로덕션 등급, 2026년 3월 오픈소스화). 이 레슨은 결정 기준을 명명하고 stdlib 라우팅 게이트웨이를 살펴본다.
...
`claude-3-5-sonnet-20251022` 대신, 당신의 코드는 `our_smart_model`이라고 말한다. 게이트웨이가 별칭을 실제 모델에 매핑한다. Anthropic이 Claude 4를 출하하면, 서버 측에서 별칭을 바꾼다; 당신의 코드는 아무것도 건드리지 않는다.
...
게이트웨이는 이를 구성(config)에 정의한다. 재시도는 예산에 카운트되어 폴백 캐스케이드가 비용을 폭발시키지 않는다.

### 시맨틱 캐싱 (Semantic caching)

동일하거나 거의 동일한 프롬프트는 제공자 대신 캐시에 적중한다. 반복되는 에이전트 루프에서의 절감은 30~60퍼센트일 수 있다. 키는 임베딩 기반이다; 거의 동일한 프롬프트는 캐시 슬롯을 공유한다.
...
모든 요청은 `provider`, `model`, `input_tokens`, `output_tokens`를 운반한다. 모델별 토큰당 가격(게이트웨이가 유지하는 가격표에서 가져옴)을 곱한다. 사용자별 / 팀별 / 프로젝트별 집계.
