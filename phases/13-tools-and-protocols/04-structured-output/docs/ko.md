...
> "모델에게 JSON을 반환해 달라고 정중히 부탁하기"는 프런티어 모델(frontier model)에서도 5~15퍼센트 실패한다. 구조화된 출력(structured output)은 제약 디코딩(constrained decoding)으로 그 간극을 메운다. 모델은 스키마를 위반할 토큰을 내보내는 것이 말 그대로 차단된다. OpenAI의 엄격 모드(strict mode), Anthropic의 스키마 타입 도구 사용, Gemini의 `responseSchema`, Pydantic AI의 `output_type`, Zod의 `.parse`는 동일한 아이디어의 다섯 가지 표면 형태다. 이 레슨은 학습자가 모든 프로덕션 추출 파이프라인(extraction pipeline)에 사용할 스키마 검증기(validator)와 엄격 모드 계약을 만든다.
...
- 올바른 제약(enum, min/max, required, pattern)을 사용해 추출 대상에 대한 JSON Schema 2020-12를 작성하기.
...
**접근법 1: JSON을 프롬프트한다.** "customer, line_items, total_usd 필드를 가진 JSON으로 답하라." 프런티어 모델에서 85~95퍼센트 동작한다. 여섯 가지 방식으로 실패한다. 빠진 중괄호, 끝에 붙은 쉼표(trailing comma), 잘못된 타입, 환각된 필드, 토큰 한도에서 잘림, "Here is your JSON:" 같은 새어 나온 산문.

**접근법 2: 생성 후 검증한다.** 자유롭게 생성하고, 파싱하고, 스키마에 대해 검증하고, 실패 시 재시도한다. 신뢰할 수 있지만 비싸다 — 모든 재시도에 비용을 치르고, 잘림(truncation) 버그는 발생할 때마다 한 턴(turn)을 추가로 들게 한다.

**접근법 3: 제약 디코딩.** 제공자가 디코드 시점에 스키마를 강제한다. 유효하지 않은 토큰은 샘플링 분포(sampling distribution)에서 마스킹된다. 출력은 파싱이 보장되고 검증이 보장된다. 실패는 한 양상으로 붕괴한다. 거부(모델이 입력이 스키마에 맞지 않는다고 판단)다.
...
- **Anthropic.** `tool_use` 입력에 대한 스키마 강제. `stop_reason: "refusal"`은 존재하지 않지만, 도구 호출 없는 `end_turn`이 그 신호다.
- **Gemini.** 요청 수준의 `responseSchema`. 2026년 Gemini는 선택된 타입에 대해 토큰 수준 문법 제약(grammar constraint)을 제공한다.
...
공통된 맥락: 스키마를 한 번 선언하고, 종단 간(end to end)으로 강제하라.

## 개념 (The Concept)

### JSON Schema 2020-12 — 공통어(lingua franca)

모든 제공자가 JSON Schema 2020-12를 받아들인다. 가장 많이 쓰는 구문은:
...
OpenAI 엄격 모드는 세 가지 요구 사항을 더한다. 모든 속성이 `required`에 나열되어야 하고, 어디서나 `additionalProperties: false`여야 하며, 해결되지 않은 `$ref`가 없어야 한다. 이것들을 어기면 API가 요청 시점에 400을 반환한다.

### Pydantic, Python 바인딩

Pydantic v2는 `model_json_schema()`를 통해 데이터클래스 형태 모델에서 JSON Schema를 생성한다. Pydantic AI는 이것을 감싸므로 다음과 같이 작성하면:
...
엄격 모드는 모델이 답하도록 강제할 수 없다. 입력이 스키마에 맞지 않으면("이메일은 송장이 아니라 시였다"), 모델은 이유를 담은 `refusal` 필드를 내보낸다. 당신의 코드는 이것을 실패가 아니라 일급(first-class) 결과로 처리해야 한다. 거부는 안전 신호로도 유용하다. 보호된 콘텐츠 이메일에서 신용카드 번호를 추출하라는 요청을 받은 모델은 안전 이유가 첨부된 거부를 반환한다.

### 공개 환경에서의 제약 디코딩

오픈 웨이트(open-weights) 구현은 세 가지 기법을 사용한다.
...
상업적 제공자는 이 중 하나를 막후에서 고른다. 2026년의 최첨단은 짧은 구조화된 출력에서는 평범한 생성보다 빠르고, 긴 것에서는 대략 같은 속도다.

### 세 가지 실패 양상

1. **파싱 오류(Parse error).** 출력이 유효한 JSON이 아니다. 엄격 모드에서는 일어날 수 없다. 비엄격 제공자에서는 여전히 일어날 수 있다.
...
엄격 모드 밖에 있을 때(Anthropic 도구 사용, 비엄격 OpenAI, 구버전 Gemini), 복구 패턴은:

```
generate -> parse -> validate -> if fail, inject error and retry, max 3x
```

한 번의 재시도면 보통 충분하다. 세 번의 재시도는 약한 모델의 변덕(flake)을 잡아낸다. 셋을 넘으면 나쁜 스키마의 신호다. 모델이 일부 입력에 대해 그것을 만족시킬 수 없으니, 프롬프트나 스키마를 고쳐야 한다.

### 소형 모델 지원

제약 디코딩은 소형 모델에서 동작한다. 문법 강제를 갖춘 30억 파라미터 오픈 모델은 구조화된 작업에서 원시 프롬프팅(raw prompting)을 쓴 700억 파라미터 모델을 능가한다. 이것이 구조화된 출력이 프로덕션에 중요한 주된 이유다. 신뢰성을 모델 크기로부터 분리한다.

## 라이브러리로 써보기 (Use It)

`code/main.py`는 stdlib로 최소한의 JSON Schema 2020-12 검증기(타입, required, enum, min/max, pattern, items, additionalProperties)를 제공한다. `Invoice` 스키마를 감싸고 가짜 LLM 출력을 검증기를 통해 돌려, 파싱 오류, 스키마 위반, 거부 경로를 보여준다. 프로덕션에서는 가짜 출력을 어떤 제공자의 실제 응답으로 교체하라.

볼 것:

- 검증기는 경로와 메시지를 가진 타입 지정 `[ValidationError]` 목록을 반환한다. 그것이 재시도 프롬프트에 드러내고 싶은 형태다.
...
4. 거부율을 측정하라. 추출 불가능해야 하는 입력 열 개(노래 가사, 수학 증명, 빈 이메일)를 구성하고 엄격 모드를 갖춘 실제 제공자를 통해 돌려라. 거부 대 환각된 출력을 세어라. 이것이 거부 인식 재시도(refusal-aware retry)를 위한 당신의 정답(ground truth)이다.

5. OpenAI의 구조화된 출력 가이드를 처음부터 끝까지 읽어라. 평범한 JSON Schema가 허용하지만 엄격 모드가 명시적으로 금지하는 구문 하나를 식별하라. 그다음 금지된 구문을 비필수적으로 사용하는 스키마를 설계하고 엄격 호환이 되도록 리팩터링하라.
