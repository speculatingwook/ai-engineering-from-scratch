...
2025년대의 `visualize_timeline` 도구는 "다음은 시간순으로 정리된 14개 노트입니다: ..."를 반환할 수 있다. 한 문단의 텍스트일 뿐이다. 사용자가 실제로 원하는 것은 상호작용형 타임라인이다. MCP 앱 이전에는 옵션이 둘뿐이었다. 클라이언트별 위젯 API(Claude 아티팩트, OpenAI Custom GPT HTML), 또는 UI 전무.

MCP 앱(SEP-1724, 2026년 1월 26일 출시)은 그 계약을 표준화한다. 도구 결과는 URI가 `ui://...`이고 MIME이 `text/html;profile=mcp-app`인 `resource`를 담는다. 호스트는 제한된 CSP를 가진 샌드박스 iframe에서, 명시적으로 부여되지 않는 한 네트워크 접근 없이 이를 렌더링한다. iframe 내부의 UI는 작은 postMessage JSON-RPC 방언으로 호스트에 메시지를 보낸다.
...
- 응답 헤더로 적용되는 서버 선언 CSP.
- 호스트 출처의 쿠키 없음, localStorage 없음.
- CSP의 `connectSrc`로 제한된 네트워크 접근.

### postMessage 프로토콜

iframe은 `window.postMessage`로 호스트와 통신한다. 작은 JSON-RPC 2.0 방언이다:
...
- JS가 `window.parent.postMessage`로 `host.callTool`을 호출한다(문서화되었지만 이 stdlib 데모에서는 비활성).
