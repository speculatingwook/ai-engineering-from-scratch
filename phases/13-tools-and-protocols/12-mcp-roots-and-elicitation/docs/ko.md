...
> 하드코딩된 경로는 사용자가 다른 프로젝트를 여는 순간 깨진다. 미리 채워진 도구 인자는 사용자가 충분히 명시하지 않을 때 깨진다. 루트(root)는 서버를 사용자가 제어하는 URI 집합으로 범위 지정한다. 유도(elicitation)는 도구 호출 중간에 일시 정지하여 폼이나 URL을 통해 사용자에게 구조화된 입력을 요청한다. 두 개의 클라이언트 프리미티브(primitive)는 흔한 MCP 실패 모드를 위한 두 가지 해결책이다. SEP-1036(URL 모드 유도, 2025-11-25)은 2026년 상반기까지 실험적이다. 의존하기 전에 SDK 버전을 확인하라.
...
서버는 반드시 루트를 경계로 취급해야 한다. 루트 집합 외부의 파일 읽기나 쓰기는 거부된다. 이것은 클라이언트가 강제하지 않지만(서버는 여전히 사용자가 신뢰한 코드다), 사양을 준수하는 서버는 이를 존중한다.

사용자가 루트를 추가하거나 제거하면 클라이언트는 `notifications/roots/list_changed`를 보낸다. 서버는 `roots/list`를 다시 호출하고 경계를 업데이트한다.

### 루트가 클라이언트 프리미티브인 이유

루트는 사용자의 동의 모델을 나타내기 때문에 클라이언트가 선언한다. 사용자가 Claude Desktop에게 "이 노트 서버에게 이 두 디렉터리에 대한 접근을 줘"라고 말한 것이다. 서버는 그 범위를 넓힐 수 없다.
...
드리프트 위험 참고: SEP-1036 응답 형태는 여전히 자리를 잡아가는 중이다. 일부 SDK는 콜백 URL을 반환하고, 다른 것은 완료 토큰을 반환한다. 프로덕션에서 URL 모드를 쓰기 전에 SDK의 릴리스 노트를 읽어라.
