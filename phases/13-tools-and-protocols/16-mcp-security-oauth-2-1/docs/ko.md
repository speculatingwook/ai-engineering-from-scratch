...
> 원격 MCP 서버는 인증(authentication)만이 아니라 인가(authorization)가 필요하다. 2025-11-25 사양은 OAuth 2.1 + PKCE + 리소스 표시자(resource indicator, RFC 8707) + 보호된 리소스 메타데이터(protected-resource metadata, RFC 9728)와 정렬된다. SEP-835는 403 WWW-Authenticate에 대한 단계 상승 인가(step-up authorization)와 함께 점진적 스코프 동의를 추가한다. 이 레슨은 단계 상승 플로(flow)를 상태 기계(state machine)로 구현하여 모든 홉(hop)을 볼 수 있게 한다.
...
- 단계 상승 인가 구현하기: 서버가 더 높은 스코프를 요청하는 WWW-Authenticate와 함께 403으로 응답하면, 클라이언트가 사용자 동의를 다시 요청하고 재시도한다.
...
- **스코프 에스컬레이션.** `notes:read`를 부여받은 노트 서버가 나중에 특정 동작을 위해 `notes:write`가 필요할 수 있다. 전체 플로를 다시 하는 대신, 단계 상승(SEP-835)이 추가 스코프를 요청한다.
- **혼동된 대리자 방지.** 클라이언트가 서버 A에 대상(audience) 범위가 지정된 토큰을 보유한다. 서버 A가 악의적이어서 그 토큰을 서버 B에 제시하려 한다. 리소스 표시자(RFC 8707)는 토큰을 의도된 대상에 고정한다.
...
클라이언트가 리소스 서버로부터 인가 서버를 발견한다. 구성을 줄인다. 클라이언트는 리소스 URL만 필요하다.
...
사용자가 `notes:read`를 부여한다. 그들은 나중에 에이전트에게 노트를 삭제하라고 요청한다. 서버가 응답한다:
...
클라이언트가 insufficient_scope 오류를 보고, 추가 스코프에 대한 동의 다이얼로그로 사용자에게 요청하고, 그것을 위한 미니 OAuth 플로를 수행하고, 새 토큰으로 요청을 재시도한다.
...
각 MCP 클라이언트는 고정된 URL에 자신의 메타데이터를 발행한다. 인가 서버는 클라이언트의 메타데이터 문서를 가져와 리다이렉트 URI와 연락처 정보를 발견할 수 있다. 이것은 수동 클라이언트 등록을 없앤다.
