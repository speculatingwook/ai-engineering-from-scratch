...
> 실제 에이전트 작업은 수 분에서 수 시간이 걸린다. CI 실행, 딥 리서치 합성, 배치 내보내기. 동기(synchronous) 도구 호출은 연결을 끊거나, 타임아웃되거나, UI를 막는다. 2025-11-25에 병합된 SEP-1686은 태스크(Task) 프리미티브(primitive)를 추가한다. 어떤 요청이든 태스크가 되도록 증강(augment)될 수 있고, 결과는 나중에 가져오거나 상태 알림(state notification)을 통해 스트리밍될 수 있다. 드리프트(drift) 위험 참고: 태스크는 2026년 상반기까지 실험적이다. SDK 표면은 여전히 사양을 중심으로 설계되는 중이다.
...
어느 것도 좋지 않다. SEP-1686은 네 번째를 추가한다. 태스크 증강. 어떤 요청이든(보통 `tools/call`) 태스크로 태깅될 수 있다. 서버는 태스크 id를 즉시 반환한다. 클라이언트는 `tasks/status`를 폴링하고 완료되면 `tasks/result`를 가져온다. 서버 측 상태는 재시작에서 살아남는다.
...
`ttl`은 상태를 유지하겠다는 서버의 약속이다. ttl 이후 태스크 결과는 폐기된다.
...
서버가 지원할 때, 클라이언트는 상태 알림을 구독할 수 있다:
...
태스크는 그 자체로 `sampling/createMessage`를 호출할 수 있다. 이것이 장기 실행 리서치 태스크가 동작하는 방식이다. 서버의 태스크 스레드가 필요에 따라 클라이언트의 모델을 샘플링(sampling)하는 동안, 클라이언트의 UI는 주기적 진행 업데이트와 함께 태스크를 `working`으로 표시한다.

### 이것이 실험적인 이유

SEP-1686은 2025-11-25에 출시되었지만 더 넓은 로드맵은 세 가지 미해결 이슈를 짚는다. 내구성 있는 구독 프리미티브, 서브태스크(subtask, 부모-자식 태스크 관계), 그리고 결과-TTL 표준화. 사양이 2026년 내내 진화할 것을 예상하라. 프로덕션 코드는 태스크를 흔한 경우에만 안정적으로 취급하고 서브태스크에 대한 미래 SDK 변경에 대비해야 한다.

## 라이브러리로 써보기 (Use It)

`code/main.py`는 내구성 있는 태스크 저장소(파일 시스템 기반)와 백그라운드 스레드에서 실행되는 `generate_report` 도구를 구현한다. 클라이언트는 도구를 호출하고, 태스크 id를 즉시 받고, 워커(worker)가 진행을 업데이트하는 동안 `tasks/status`를 폴링하고, 완료되면 `tasks/result`를 가져온다. 취소가 동작한다. 크래시 복구는 워커 스레드를 죽이고 상태를 다시 로드하여 시뮬레이션된다.

살펴볼 것:

- `/tmp/lesson-13-tasks/<id>.json`에 영속화된 태스크 상태 JSON.
- 워커 스레드가 `progress` 필드를 업데이트한다. 폴링이 그것이 전진하는 것을 보여준다.
- 클라이언트 측 취소가 이벤트를 설정한다. 워커가 확인하고 일찍 종료한다.
