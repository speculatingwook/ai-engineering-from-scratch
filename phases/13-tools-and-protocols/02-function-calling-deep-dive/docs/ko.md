...
> 세 프런티어 제공자(frontier provider)는 2024년에 동일한 도구 호출 루프로 수렴했다가, 그 외의 모든 것에서는 갈라졌다. OpenAI는 `tools`와 `tool_calls`를 사용한다. Anthropic은 `tool_use`와 `tool_result` 블록을 사용한다. Gemini는 `functionDeclarations`와 고유 id 상관(correlation)을 사용한다. 이 레슨은 세 가지를 나란히 비교(diff)하여, 한 제공자에서 출시한 코드가 다른 제공자로 포팅할 때 깨지지 않도록 한다.
...
함수 호출 요청의 형태는 제공자별로 다르다. 2026년 프로덕션 스택에서 가져온 세 가지 구체적 예시다.

**OpenAI Chat Completions / Responses API.** `tools: [{type: "function", function: {name, description, parameters, strict}}]`를 전달한다. 모델의 응답은 `choices[0].message.tool_calls: [{id, type: "function", function: {name, arguments}}]`를 담으며, 여기서 `arguments`는 당신이 파싱해야 하는 JSON 문자열이다. 엄격 모드(`strict: true`)는 제약 디코딩(constrained decoding)을 통해 스키마 준수를 강제한다.
...
이 레슨은 세 형식을 하나의 표준(canonical) 도구 선언으로 통합하고 가장자리(edge)에서 라우팅하는 번역기를 만든다. Phase 13 · 17은 동일한 패턴을 LLM 게이트웨이로 일반화한다.
...
- **Anthropic.** 요청당 도구 64개. 스키마 깊이는 사실상 무제한이나 실용 한도는 10. 엄격 모드 플래그 없음. 스키마는 계약이고 모델은 준수하는 경향이 있다.
- **Gemini.** 요청당 함수 64개. 스키마 타입은 OpenAPI 3.0 부분집합(JSON Schema 2020-12에서 약간 분기됨). 병렬 호출은 Gemini 3부터 고유 id.

### `tool_choice` 동작

모두가 지원하는 세 모드, 이름은 다르다.

- **Auto.** 모델이 도구나 텍스트를 고른다. 기본값.
- **Required / Any.** 모델이 최소 하나의 도구를 호출해야 한다.
- **None.** 모델이 도구를 호출해서는 안 된다.

여기에 각 제공자에게 고유한 한 모드가 더 있다.

- **OpenAI.** 이름으로 특정 도구를 강제한다.
- **Anthropic.** 이름으로 특정 도구를 강제한다. `disable_parallel_tool_use` 플래그가 단일 대 다중을 분리한다.
- **Gemini.** `mode: "VALIDATED"`는 모델 의도와 무관하게 모든 응답을 스키마 검증기(validator)를 통해 라우팅한다.

### 병렬 호출 (Parallel calls)

OpenAI의 `parallel_tool_calls: true`(기본값)는 하나의 assistant 메시지에 여러 호출을 내보낸다. 모두 실행하고 `tool_call_id`당 하나의 항목을 담은 배치(batched) tool-역할 메시지로 답한다. Anthropic은 과거에 단일 호출을 했고, `disable_parallel_tool_use: false`(Claude 3.5 시점 기본값)가 다중을 활성화한다. Gemini 2는 병렬 호출을 허용했지만 안정적인 id를 주지 않았고, Gemini 3는 UUID를 추가하여 순서가 뒤바뀐 응답이 깔끔하게 상관된다.
...
- **OpenAI (비엄격).** 모델이 `arguments: "{bad json}"`을 반환하고, 당신의 JSON 파싱이 실패하면, 오류 메시지를 주입하고 다시 호출한다.
...
당신의 코드에서 표준 도구 선언은 다음과 같이 생겼다(형태는 당신이 고른다).
...
`code/main.py`는 하나의 표준 `Tool` 데이터클래스(dataclass)와 OpenAI, Anthropic, Gemini 선언 JSON을 내보내는 세 개의 번역기를 정의한다. 그다음 각 형태의 손으로 만든 제공자 응답을 동일한 표준 호출 객체로 파싱하여, 의미론이 껍질 아래에서는 동일함을 보여준다. 이를 돌리고 세 선언을 나란히 비교하라.
