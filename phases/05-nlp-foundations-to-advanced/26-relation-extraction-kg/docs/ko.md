...
> NER이 엔티티(entity)를 찾았다. 엔티티 링킹(entity linking)이 그것들을 고정했다. 관계 추출(relation extraction)은 그것들 사이의 간선(edge)을 찾는다. 지식 그래프(knowledge graph)는 노드(node), 간선, 그리고 그 출처(provenance)의 합이다.
...
관계 추출(RE)은 자유 텍스트를 구조화된 트리플(triple) `(subject, relation, object)`로 바꾼다. 코퍼스 전체에 걸쳐 집계하면 지식 그래프가 된다. 집계하고 쿼리하면 RAG, 분석, 또는 컴플라이언스 감사를 위한 추론 기반이 된다.

2026년의 문제: LLM은 관계를 열정적으로 추출한다. 너무 열정적으로. 원본 텍스트가 뒷받침하지 않는 트리플을 환각(hallucination)한다. 출처가 없으면 진짜 트리플과 그럴듯한 허구를 구별할 수 없다. 2026년의 답은 AEVS 스타일의 앵커-검증(anchor-and-verify) 파이프라인이다.
...
1. **규칙 / 패턴 기반.** Hearst 패턴: "X such as Y" → `(Y, isA, X)`. 더해 손으로 만든 정규식(regex). 부서지기 쉽고, 정밀하며, 설명 가능하다.
2. **지도 학습 분류기.** 문장 내 두 엔티티 언급(mention)이 주어지면, 고정된 집합에서 관계를 예측한다. TACRED, ACE, KBP로 학습. 2015~2022년의 표준.
3. **생성형 LLM.** 모델에 트리플을 내보내도록 프롬프트한다. 바로 동작한다. 출처가 필요하며, 그렇지 않으면 그럴듯해 보이는 쓰레기를 환각한다.
...
- **보완(Supplement).** 커버리지 패스가 앵커된 스팬이 빠지지 않도록 보장한다.

환각이 급격히 줄어든다. 연산량이 더 필요하지만 감사 가능하다.
...
프로덕션(production) KG는 보통 섞는다. 발견을 위한 open IE를 쓴 다음, 메인 그래프에 병합하기 전에 관계를 닫힌 온톨로지로 표준화한다.
...
전체 장난감 추출기는 `code/main.py`를 보라. Hearst 패턴은 디버깅 가능하기 때문에 여전히 도메인 특화 파이프라인에서 출시된다.
...
REBEL은 seq2seq 관계 추출기다. 텍스트 입력, 트리플 출력, 이미 Wikidata 속성 id 형태다. 원거리 지도(distant-supervision) 데이터로 파인튜닝(fine-tuning)되었다. 표준 오픈 웨이트(open-weights) 베이스라인(baseline)이다.
...
반환된 모든 스팬을 원본에 대조해 검증하라. `text[start:end] != triple_entity`인 것은 거부하라. 이것이 AEVS의 "검증" 단계를 최소한의 형태로 구현한 것이다.
...
표준화는 종종 엔지니어링 작업의 60~80%다. 그만큼의 예산을 잡아라.
...
이것이 모든 RAG-over-KG 시스템의 원자다. RDF 트리플 저장소(Blazegraph, Virtuoso), 속성 그래프(Neo4j), 또는 벡터 증강 그래프 저장소로 규모를 확장하라.

## 함정 (Pitfalls)

- **RE 이전의 상호참조(coreference).** "He founded Apple" — RE는 "he"가 누구인지 알아야 한다. 먼저 상호참조를 실행하라(레슨 24).
- **엔티티 표준화.** "Apple Inc"와 "Apple"은 같은 노드로 해결되어야 한다. 먼저 엔티티 링킹(레슨 25).
...
- **시간 오류.** "Tim Cook is CEO of Apple" — 지금은 참이지만 2005년에는 거짓이다. 많은 관계가 시간으로 한정된다. 한정어(Wikidata의 `P580` 시작 시간, `P582` 종료 시간)를 사용하라.
- **도메인 불일치.** REBEL은 Wikipedia로 학습되었다. 법률, 의료, 과학 텍스트는 종종 도메인 파인튜닝된 RE 모델이 필요하다.
...
2. **보통.** 같은 문장에 REBEL(또는 작은 LLM)을 사용하라. 트리플을 비교하라. 어느 추출기가 더 높은 정밀도를 갖는가? 더 높은 재현율은?
3. **어려움.** AEVS 파이프라인을 만들어라: LLM으로 추출 + 원본에 대한 스팬 검증. Wikipedia 스타일 문장 50개에서 검증 단계 이전 대비 이후의 환각률을 측정하라.
