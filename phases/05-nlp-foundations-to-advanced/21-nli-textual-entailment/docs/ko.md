...
요약기를 만들었다고 하자. 요약문이 나왔다. 그 요약에 환각(hallucination)이 들어있지 않다는 것을 어떻게 아는가?

챗봇을 만들었다고 하자. 챗봇이 "예"라고 답했다. 그 답이 검색된 단락(passage)으로 뒷받침된다는 것을 어떻게 아는가?

1만 개의 뉴스 기사를 주제별로 분류해야 한다. 학습 레이블(label)은 하나도 없다. 모델을 재사용할 수 있을까?
