...
NER은 모든 구조화 추출 파이프라인(pipeline) 아래의 일꾼이다. 이력서 파싱, 규정 준수 로그 스캔, 의료 기록 익명화, 검색 질의 이해, 챗봇 응답을 위한 근거 부여(grounding), 법률 계약 추출. 결코 눈에 띄지 않지만, 항상 그것에 의존한다.

이 레슨은 고전적 경로(규칙 기반, HMM, CRF)를 거쳐 현대적 경로(BiLSTM-CRF, 그다음 트랜스포머)로 나아간다. 각 단계는 그 이전 단계의 특정 한계를 해결한다. 그 패턴이 이 레슨의 핵심이다.
...
프로덕션 가제티어는 위키피디아와 DBpedia에서 스크랩한 수백만 개의 항목을 가진다. 커버리지는 좋다. 중의성 해소(`Apple` 회사 대 과일)는 형편없다. 그래서 통계적 모델이 이겼다.
...
`c1`과 `c2`는 L1과 L2 규제(regularization)다. `all_possible_transitions=True`는 모델이 불법적인 시퀀스(예: `O` 뒤의 `I-ORG`)가 일어날 법하지 않음을 학습하게 하는데, 이것이 CRF가 당신이 제약을 작성하지 않고도 BIO 일관성을 강제하는 방식이다.
...
- **RAG를 이용한 동적 프롬프팅.** 추론 호출마다 작은 주석 시드 집합에서 가장 유사한 레이블된 예시를 검색해, 퓨샷 프롬프트를 즉석에서 구성한다. 2026년 벤치마크(benchmark)에서 이는 정적 프롬프팅 대비 GPT-4 생의학 NER F1을 11-12% 끌어올린다.
...
- **도메인 시프트.** 법률 계약에 대해 CoNLL로 학습된 NER은 가제티어보다 못한 성능을 낸다. 당신의 도메인으로 파인튜닝하라.
