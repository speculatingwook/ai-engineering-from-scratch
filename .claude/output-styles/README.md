...
`.claude/settings.json` sets `outputStyle: fluent-korean`, so any Claude Code
session opened in this repository writes Korean that keeps its particles,
...
`.claude/output-styles/` are committed with the repository, apply to everyone who
clones it, and can carry local additions. Two such additions are appended to both
styles here, taken verbatim from the upstream README's optional blocks:
...
Switch with `/output-style`, or by editing `.claude/settings.json`. An output
style is read when a session starts, so run `/clear` or open a new session after
changing it.
